...
import com.twitter.scalding.{ Config, Execution, ExecutionCounters, CancellationHandler}
...
class SparkWriter(val sparkMode: SparkMode) extends Writer {
  ...
  def execute(
    conf: Config,
    writes: List[ToWrite[_]])(implicit cec: ExecutionContext): (Future[(Long, ExecutionCounters)], CancellationHandler) = {
    ...
    (Future.traverse(acts) { fn => fn() }.map(_ => (id, ExecutionCounters.empty)), CancellationHandler.empty)
  }
}
