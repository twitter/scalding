...
object Iterators {
  ...
  /**
   * Partitions the iterator into runs of equivalent keys, then returns an iterator of each distinct key
   * followed by an sub-iterator of its values.
   *
   * For example:
   *
   * val lst = (1 to 8).map(i => (i / 3, i)).toList // List((0, 1), (0, 2), (1, 3), (1, 4), (1, 5), (2, 6),
   * (2, 7), (2, 8))
   *
   * groupSequential(lst.iterator).map(_.toList).toList // List((0, List(1, 2)), (1, List(3, 4, 5)), (2,
   * List(6, 7, 8)))
   *
   * groupSequential() does not load the iterator's contents into memory more than is absolutely necessary.
   * This means it is safe to use on very large (or potentially infinite) iterators as long as the downstream
   * consumer handles the results carefully.
   *
   * Note that the sub-iterators are fragile. That means that as soon as you call .hasNext or .next on the
   * top-level iterator, the previous sub-iterator is invalidated (and will become empty). A consequence of
   * this is that you can only operate on one sub-iterator at a time.
   *
   * Laws:
   *
   * 0. groupSequential(it).map(_._2.length).sum ~ it.length
   *
   *   1. groupSequential(it).flatMap { case (k, vs) => vs.map(v => (k, v)) } ~ it
   *
   * 2. given xs = lst.sortBy(_._1): groupSequential(xs.iterator).map { case (k, vs) => (k, vs.toList)
   * }.toList ~ xs.groupBy(_._1).mapValues(_.map(_._2)).toList.sortBy(_._1)
   */
   ...
   *
   * This process always requires one item worth of look-ahead, so this class' constructor assumes the caller
   * has already pulled `(k0, v0)` from the front of `it`.
   */
