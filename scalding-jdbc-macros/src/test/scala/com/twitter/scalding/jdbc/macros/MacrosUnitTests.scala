...
class JdbcMacroUnitTests extends WordSpec with Matchers {
  ...
  def isJDBCTypeInfoAvailable[T](implicit proof: JDBCTypeInfo[T] = dummy.asInstanceOf[JDBCTypeInfo[T]]) {
    proof shouldBe a[MacroGenerated]
  ...
  "Produces the JDBCTypeInfo" should {
    // explictly just call this to get a compiler error
    JDBCMacro.toJDBCTypeInfo[User]
    ...
    assert(JDBCMacro.toJDBCTypeInfo[User].columnDefn.columns.toList === expectedColumns)
  ...
  "Big Jdbc Test" should {
    ...
    // explictly just call this to get a compiler error
    JDBCMacro.toJDBCTypeInfo[ExhaustiveJdbcCaseClass]
    // ensure the implicit fires
    ...
    assert(JDBCMacro.toColumnDefinitionProvider[ExhaustiveJdbcCaseClass].columns.toList === expectedColumns)
  }
