...
private[handler] abstract class AnnotationHelper {
  ...
  def sizeAnnotation: scala.util.Try[(AnnotationHelper, SizeAnno)] =
    consume[SizeAnno](typeOf[com.twitter.scalding.jdbc.macros.size])(_.map(o => WithSize(o.get)).getOrElse(WithoutSize))

  def textAnnotation: scala.util.Try[(AnnotationHelper, TextAnno)] =
    consume(typeOf[com.twitter.scalding.jdbc.macros.text])(_.map(_ => WithText).getOrElse(WithoutText))

  def varcharAnnotation: scala.util.Try[(AnnotationHelper, VarcharAnno)] =
    consume(typeOf[com.twitter.scalding.jdbc.macros.varchar])(_.map(_ => WithVarchar).getOrElse(WithoutVarchar))

  def dateAnnotation: scala.util.Try[(AnnotationHelper, DateAnno)] =
    consume(typeOf[com.twitter.scalding.jdbc.macros.date])(_.map(_ => WithDate).getOrElse(WithoutDate))
