...
class BeamBackendTests extends FunSuite with BeforeAndAfter {
  ...
  test("priorityQueue operations") {
    ...
    /**
     * @note
     *   we are not extending support for `sortedTake` and `sortedReverseTake`, since both of them uses
     *   [[com.twitter.algebird.mutable.PriorityQueueMonoid.plus]] which mutates input value in pipeline and
     *   Beam does not allow mutations to input during transformation
     */
    val test = Try {
      beamMatchesSeq(
        TypedPipe
