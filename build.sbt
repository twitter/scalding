...
val sharedSettings = assemblySettings ++ Seq(
    ...
    val excludes = Set("jsp-api-2.1-6.1.14.jar",
                       "jsp-2.1-6.1.14.jar",
                       "jasper-compiler-5.5.12.jar",
                       "janino-2.5.16.jar")
    (fullClasspath in assembly).value filter { jar =>
...
def youngestForwardCompatible(subProj: String) =
  Some(subProj)
    ...
    .map { s =>
      "com.twitter" %% (s"scalding-$s") % "0.16.0"
    }
...
lazy val scaldingSerialization = module("serialization").settings(
  libraryDependencies ++= Seq(
    "org.scala-lang" % "scala-reflect" % scalaVersion.value
  ) ++ (if (isScala210x(scalaVersion.value)) Seq("org.scalamacros" %% "quasiquotes" % "2.0.1")
        else Seq()),
  addCompilerPlugin("org.scalamacros" % "paradise" % paradiseVersion cross CrossVersion.full)
)
