package com.twitter.scalding.hraven.memory_estimation
...
import com.twitter.scalding.memory_estimation.{ FlowStepMemoryHistory, HistoryMemoryEstimator, MemoryService, Task }
import com.twitter.scalding.reducer_estimation.{ FlowStepKeys, FlowStrategyInfo }
import scala.util.{ Failure, Success, Try }

object HRavenClient {
  import HRavenMemoryService.jobConfToRichConfig

  val apiHostnameKey = "hraven.api.hostname" // hraven.devel.vchannapattan.service.smf1.twitter.com
  val clientConnectTimeoutKey = "hraven.client.connect.timeout"
  val clientReadTimeoutKey = "hraven.client.read.timeout"

  private final val clientConnectTimeoutDefault = 120000
  private final val clientReadTimeoutDefault = 120000
...
/**
 * Mixin for MemoryEstimators to give them the ability to query hRaven for
 * info about past runs.
 */
object HRavenMemoryService extends MemoryService {

  private val LOG = LoggerFactory.getLogger(this.getClass)

  private val TaskCounterGroup = "org.apache.hadoop.mapreduce.TaskCounter"
  private val CommittedHeapBytes = "COMMITTED_HEAP_BYTES"
  private val CpuMs = "CPU_MILLISECONDS"
  private val PhysicalMemoryBytes = "PHYSICAL_MEMORY_BYTES"
  private val GCTimeMs = "GC_TIME_MILLIS"
  ...
  private val TaskDetailFields = List("taskType").asJava

  private val TaskCounterFields = List(
    ...
    "org.apache.hadoop.mapreduce.TaskCounter.CPU_MILLISECONDS").asJava
  ...
  private def fetchSuccessfulFlows(client: HRavenRestClient, cluster: String, user: String, batch: String, signature: String, max: Int, nFetch: Int): Try[Seq[Flow]] =
    Try(client.fetchFlowsWithConfig(cluster, user, batch, signature, nFetch, RequiredJobConfigs: _*))
      .flatMap { flows =>
        Try {
          ...
          flows.asScala.foreach { flow =>
            flow.getJobs.asScala.foreach { job =>
              ...
              val tasks = client.fetchTaskDetails(flow.getCluster, job.getJobId, TaskDetailFields, TaskCounterFields)
              job.addTasks(tasks)
            }
          }
        ...
        case e: IOException =>
          LOG.error("Error making API request to hRaven. HRavenMemoryService will be disabled.")
          Failure(e)
      }
  ...
  private def toFlowStepMemoryHistory(keys: FlowStepKeys, step: JobDetails, tasks: Seq[Task]) =
    FlowStepMemoryHistory(
      keys = keys,
...
class HRavenMemoryEstimator extends HistoryMemoryEstimator {
  override val memoryService = HRavenMemoryService
}
