...
object HRavenHistoryService {
  ...
  case class RichConfig(conf: JobConf) {
    ...
    /**
     * Try fields in order until one returns a value. Logs a warning if nothing was found.
     */
...
trait HRavenHistoryService extends HistoryService {
  ...
  /**
   * Fetch flows until it finds one that was successful (using "HdfsBytesRead > 0" as a marker for successful
   * jobs since it seems that this is only set on completion of jobs)
   *
   * TODO: query hRaven for successful jobs (first need to add ability to filter results in hRaven REST API)
   */
            ...
            flow.getJobs.asScala
              ...
              .foreach { job =>
                ...
                val tasks = if (counterFields.isEmpty) {
                  ...
                  client.fetchTaskDetails(
                    flow.getCluster,
                    job.getJobId,
                    detailFields.asJava,
                    counterFields.asJava
                  )
                }
                job.addTasks(tasks)
              }
  ...
  /**
   * Fetch info from hRaven for the last time the given JobStep ran. Finds the last successful complete flow
   * and selects the corresponding step from it.
   *
   * @param step
   *   FlowStep to get info for
   * @return
   *   Details about the previous successful run.
   */
    ...
    fetchPastJobDetails(info.step, maxHistory).map { history =>
      for {
        ...
        tasks = step.getTasks.asScala.flatMap { taskDetails =>
          details(taskDetails).zip(counters(taskDetails.getCounters)).map { case (details, counters) =>
  ...
  private def getCounterValueAsLong(
      counters: CounterMap,
