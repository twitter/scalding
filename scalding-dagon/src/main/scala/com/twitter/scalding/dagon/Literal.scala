...
sealed trait Literal[N[_], T] extends Serializable { self: Product =>
  ...
  /**
   * Here we memoize as we check equality and always check reference equality first. This can dramatically
   * improve performance on graphs that merge back often
   */
...
object Literal {
  ...
  sealed case class Binary[N[_], T1, T2, T3](
      arg1: Literal[N, T1],
      arg2: Literal[N, T2],
      fn: (N[T1], N[T2]) => N[T3]
  ) extends Literal[N, T3]

  sealed case class Variadic[N[_], T1, T2](args: List[Literal[N, T1]], fn: List[N[T1]] => N[T2])
      extends Literal[N, T2]

  /**
   * This evaluates a literal formula back to what it represents being careful to handle diamonds by creating
   * referentially equivalent structures (not just structurally equivalent)
   */
  ...
  /**
   * Note that this is a def, not a val, so the cache only lives as long as a single outermost equality check
   */
