...
object RealNumbers {
  ...
  sealed abstract class Real { self: Product =>
    ...
    /**
     * This multiplicativily increases the size of the Real, be careful, consider (a1 + a2 + .. an) * (b1 + b2
     * + .. + bk) we wind up with n*k items from n + k. We can wind up exponentially larger: (a0 + a1)*(b0 +
     * b1)*.... will be exponentially larger number of terms after expand
     */
    def expand: Real =
      this match {
    ...
    /**
     * If this divides the current value return a Some(res) such that res * r0 == this
     */
  ...
  val realParser: Parser[Real] = {
    ...
    val constP =
      (Parser
        .chr('-')
        .?
        .zip(intP.zip((Parser.chr('.') *> Parser.Rep(digit)).?)))
        .map {
...
class RealNumberTest extends FunSuite {
  ...
  test("optimizeAll reduces cost") {
    ...
    val strongCases = List("((x*1) + (x*2))", "((x*1) + (x*2) + (y*3))", "(1 + 2)")
  ...
  test("optimization does not change evaluation") {
    ...
    forAll(completeEval, implicitly[Arbitrary[Set[Rule[RealN]]]].arbitrary) { case ((r, m), rules) =>
    ...
    // past failures here
    List(
      ("((-986.0*x) + (-325.0*363.0*z) + (530.0*928.0*x))", Map("x" -> 1.0, "z" -> 10.0), allRules0 :: Nil),
      (
        "(q + q + r + x + (-755.0*-394.0*674.0))",
        Map("x" -> 0.0, "q" -> 0.0, "r" -> 3.3953184045350335e-5),
        ReduceProd :: Nil
      ),
  ...
  test("optimization de-foils easy cases") {
    ...
    forAll(prod) { r: Real =>
      ...
      assert(
        optR.cost <= cost0,
        s"$r ($cost0) optimized to $optR expanded to: $expanded expanded cost: $expCost"
      )
    }
  }
  ...
  test("optimization nearly de-foils") {
    val sum = Gen
      .choose(2, 4)
      .flatMap(Gen.listOfN(_, Real.genReal(0)))
      .map(ls => Real.sum(SortedList(ls: _*)))
    val prod = Gen
      .choose(2, 3)
      .flatMap(Gen.listOfN(_, sum))
