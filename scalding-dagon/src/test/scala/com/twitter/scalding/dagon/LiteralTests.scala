...
object LiteralTests extends Properties("Literal") {
  ...
  def transitiveClosure[N[_]](
      l: Literal[N, _],
      acc: Set[Literal[N, _]] = Set.empty[Literal[N, _]]
  ): Set[Literal[N, _]] = l match {
    ...
    def append(cnt: Int, items: List[Literal[Box, Int]]): Gen[List[Literal[Box, Int]]] =
      if (cnt > 0) {
        ...
        val hGen: Gen[Literal[Box, Int]] =
          if (items.nonEmpty) {
            val inner = Gen
              .oneOf(items.flatMap(transitiveClosure[Box](_)))
  ...
  property("Literal.evaluate must match simple explanation") = forAll(genLiteral) { (l: Literal[Box, Int]) =>
    l.evaluate == slowEvaluate(l)
  }

  property("equality is transitive") = forAll(genLiteral, genLiteral, genLiteral) { (a, b, c) =>
    if (a == b) {
  ...
  property("binary equality regression check") = forAll(genLiteral, genLiteral) { (a, b) =>
    if (a != b) {
  ...
  property("reflexive equality") = forAll(genLiteral, genLiteral)((a, b) => (a == b) == (b == a))

  property("equality spec") = forAll(genLiteral, genLiteral) { (a, b) =>
    (a, b) match {
