...
/**
 * This Source writes out the TupleEntry as a simple JSON object, using the field names as keys and the string
 * representation of the values.
 *
 * TODO: it would be nice to have a way to add read/write transformations to pipes that doesn't require
 * extending the sources and overriding methods.
 *
 * @param failOnEmptyLines
 *   When set to false, it just skips empty lines instead of failing the jobs. Defaults to true for backwards
 *   compatibility.
 */
case class JsonLine(
    p: String,
  ...
  override def transformForWrite(pipe: Pipe) = pipe.mapTo(fields -> 'json) { t: TupleEntry =>
 ...
 */
object JsonLine
    extends scala.runtime.AbstractFunction5[String, Fields, SinkMode, Boolean, Boolean, JsonLine]
    with Serializable
