...
/**
 * This file contains workarounds for PARQUET-346, everything in it should be removed once that bug is fixed
 * in upstream parquet.
 *
 * The root issue is that TBaseRecordConverter passes a schema based on the file metadata to
 * ThriftRecordConverter that may be missing structOrUnionType metadata. This metadata is not actually needed,
 * but parquet currently throws if it's missing. The (temporary) "fix" is to populate this metadata by setting
 * all structOrUnionType fields to UNION.
 */
...
/**
 * Same as TBaseRecordConverter with one important (subtle) difference. It passes a repaired schema
 * (StructType) to ThriftRecordConverter's constructor. This is important because older files don't contain
 * all the metadata needed for ThriftSchemaConverter to not throw, but we can put dummy data in there because
...
/**
 * Takes a ThriftType with potentially missing structOrUnionType metadata, and makes a copy that sets all
 * StructOrUnionType metadata to UNION
 */
