...
object TypedParquet {
  ...
  /**
   * Create readable typed parquet source. Here is an example: import
   * com.twitter.scalding.parquet.tuple.macros.Macros._ val parquetTuple =
   * TypedParquet[SampleClass](Seq(outputPath))
   *
   * @param paths
   *   paths of parquet I/O
   * @tparam T
   *   Tuple type
   * @return
   *   a typed parquet source.
   */
  ...
  def apply[T](path: String)(implicit readSupport: ParquetReadSupport[T]): TypedParquet[T] =
    apply[T](Seq(path))
  ...
  def apply[T](paths: Seq[String], fp: FilterPredicate)(implicit
      readSupport: ParquetReadSupport[T]
  ): TypedParquet[T] =
    new TypedFixedPathParquetTuple[T](paths, readSupport, null) {
...
object TypedParquetSink {
  ...
  /**
   * Create typed parquet sink. Here is an example: import com.twitter.scalding.parquet.tuple.macros.Macros._
  ...
  def apply[T](path: String)(implicit writeSupport: ParquetWriteSupport[T]): TypedParquet[T] =
    apply[T](Seq(path))
}
 ...
 */
trait TypedParquet[T] extends FileSource with Mappable[T] with TypedSink[T] with HasFilterPredicate {
...
class TypedFixedPathParquetTuple[T](
