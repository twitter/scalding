...
abstract class ParquetSourcesTestsBase extends WordSpec {
  ...
  def testReturnProvidedColumns[S <: Source with HasColumnProjection](
    ...
    "correctly format globs into parquet's expected format " + expected in {
      verifyParquetStringFormat(src.columnProjectionString.get.asSemicolonString,
...
class ParquetSourcesTests extends ParquetSourcesTestsBase {
  ...
  "DailySuffixParquetThrift" should {
    ...
    testReturnProvidedFilter(new DailySuffixParquetThrift[MockTBase](path, dateRange) {
  ...
  "HourlySuffixParquetThrift" should {
    ...
    testReturnProvidedFilter(new HourlySuffixParquetThrift[MockTBase](path, dateRange) {
      override val withFilter: Option[FilterPredicate] = Some(filter1)
    })
  ...
  "FixedPathParquetThrift" should {
    ...
    testReturnProvidedFilter(new FixedPathParquetThrift[MockTBase](path, path, path) {
      override val withFilter: Option[FilterPredicate] = Some(filter1)
    })
  ...
  "DailySuffixParquetTuple" should {
    ...
    testReturnProvidedFilter(new DailySuffixParquetTuple(path, dateRange, fields) {
      override val withFilter: Option[FilterPredicate] = Some(filter1)
    })
  }
  ...
  "HourlySuffixParquetTuple" should {
    ...
    testReturnProvidedFilter(new HourlySuffixParquetTuple(path, dateRange, fields) {
      override val withFilter: Option[FilterPredicate] = Some(filter1)
    })
  }
