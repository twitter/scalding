 ...
 * mean that there is no boxing and if the JIT does its work, Writer should compose well (via collections,
...
object Writer {
  ...
  def write[@specialized(Boolean, Byte, Short, Int, Long, Float, Double) T](os: OutputStream, t: T)(implicit
      w: Writer[T]
  ): Unit =
    w.write(os, t)
  /*
