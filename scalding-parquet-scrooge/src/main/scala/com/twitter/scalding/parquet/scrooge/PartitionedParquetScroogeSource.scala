 ...
 * scrooge object. `P` must be either a String or a tuple of Strings. Below is an example.
 * {{{
 ...
 * }}}
 *
 * For reading it produces a pair `(P, T)` where `P` is the partition data, `T` is the corresponding scrooge
 ...
 * }}}
 */
case class PartitionedParquetScroogeSource[P, T <: ThriftStruct](path: String, template: String)(implicit
    val ct: ClassTag[T],
