...
object KeyedListLike {
  ...
  ): TypedPipe[(K, V)] = keyed.toTypedPipe

  implicit def toTypedPipeKeyed[K, V, S[K, +V] <: KeyedListLike[K, V, S]](
      keyed: KeyedListLike[K, V, S]
  ): TypedPipe.Keyed[K, V] =
...
/**
 * This is for the case where you don't want to expose any structure but the ability to operate on an iterator
 * of the values
 */
trait KeyedList[K, +T] extends KeyedListLike[K, T, KeyedList]

/**
 * Represents sharded lists of items of type T There are exactly two fundamental operations: toTypedPipe:
 * marks the end of the grouped-on-key operations. mapValueStream: further transforms all values, in order,
 * one at a time, with a function from Iterator to another Iterator
 */
trait KeyedListLike[K, +T, +This[K, +T] <: KeyedListLike[K, T, This]] extends Serializable {

  /**
   * End of the operations on values. From this point on the keyed structure is lost and another shuffle is
   * generally required to reconstruct it
   */
  def toTypedPipe: TypedPipe[(K, T)]

  /**
   * This is like take except that the items are kept in memory and we attempt to partially execute on the
   * mappers if possible For very large values of n, this could create memory pressure. (as you may aggregate
   * n items in a memory heap for each key) If you get OOM issues, try to resolve using the method `take`
   * instead.
   */
  ...
  /**
   * Operate on an Iterator[T] of all the values for each key at one time. Prefer this to toList, when you can
   * avoid accumulating the whole list in memory. Prefer sum, which is partially executed map-side by default.
  ...
  /**
   * .filter(fn).toTypedPipe == .toTypedPipe.filter(fn) It is generally better to avoid going back to a
   * TypedPipe as long as possible: this minimizes the times we go in and out of cascading/hadoop types.
   */
  ...
  /**
   * This is just short hand for mapValueStream(identity), it makes sure the planner sees that you want to
   * force a shuffle. For expert tuning
   */
  ...
  /**
   * This is a special case of mapValueStream, but can be optimized because it doesn't need all the values for
   * a given key at once. An unoptimized implementation is: mapValueStream { _.map { fn } } but for Grouped we
   * can avoid resorting to mapValueStream
   */
  ...
  /**
   * Add all items according to the implicit Semigroup If there is no sorting, we default to assuming the
   * Semigroup is commutative. If you don't want that, define an ordering on the Values, sort or
   * .forceToReducers.
   *
  ...
  /**
   * reduce with fn which must be associative and commutative. Like the above this can be optimized in some
   * Grouped cases. If you don't have a commutative operator, use reduceLeft
   */
  def reduce[U >: T](fn: (U, U) => U): This[K, U] =
    sum(SemigroupFromFn(fn))

  /**
   * Take the largest k things according to the implicit ordering. Useful for top-k without having to call
   * ord.reverse
   */
  ...
  /**
   * This implements bottom-k (smallest k items) on each mapper for each key, then sends those to reducers to
   * get the result. This is faster than using .take if k * (number of Keys) is small enough to fit in memory.
   */
  ...
  /**
   * Folds are composable aggregations that make one pass over the data. If you need to do several custom
   * folds over the same data, use Fold.join and this method
   */
  ...
  /**
   * Similar to reduce but always on the reduce-side (never optimized to mapside), and named for the scala
   * function. fn need not be associative and/or commutative. Makes sense when you want to reduce, but in a
   * particular sorted order. the old value comes in on the left.
   */
  def reduceLeft[U >: T](fn: (U, U) => U): This[K, U] =
    sumLeft[U](SemigroupFromFn(fn))

  /**
   * Semigroups MAY have a faster implementation of sum for iterators, so prefer using sum/sumLeft to
   * reduce/reduceLeft
   */
  ...
  /**
   * For each key, give the number of unique values. WARNING: May OOM. This assumes the values for each key
   * can fit in memory.
   */
  ...
  /**
   * For each key, remove duplicate values. WARNING: May OOM. This assumes the values for each key can fit in
   * memory.
   */
  def distinctValues: This[K, T] = toSet[T].flattenValues

  /**
   * AVOID THIS IF POSSIBLE For each key, accumulate all the values into a List. WARNING: May OOM Only use
   * this method if you are sure all the values will fit in memory. You really should try to ask why you need
   * all the values, and if you want to do some custom reduction, do it in mapGroup or mapValueStream
   *
   * This does no map-side aggregation even though it is a Monoid because toList does not decrease the size of
   * the data at all, so in practice it only wastes effort to try to cache.
   */
  def toList: This[K, List[T]] = mapValueStream(ToList[T]())

  /**
   * AVOID THIS IF POSSIBLE Same risks apply here as to toList: you may OOM. See toList. Note that toSet needs
   * to be parameterized even though toList does not. This is because List is covariant in its type parameter
   * in the scala API, but Set is invariant. See:
  ...
  /**
   * Use this to error if there is more than 1 value per key Using this makes it easier to detect when data
   * does not have the shape you expect and to communicate to scalding that certain optimizations are safe to
   * do
   *
   * Note, this has no effect and is a waste to call after sum because it is true by construction at that
   * point
   */
