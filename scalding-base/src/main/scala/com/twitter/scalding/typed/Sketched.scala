...
import com.twitter.algebird.{Batched, Bytes, CMS}
 ...
 */
case class Sketched[K, V](pipe: TypedPipe[(K, V)], numReducers: Int, delta: Double, eps: Double, seed: Int)(
    implicit
  ...
  lazy val sketch: TypedPipe[CMS[Bytes]] = {
    ...
    lazy implicit val batchedSG: com.twitter.algebird.Semigroup[Batched[CMS[Bytes]]] =
      Batched.compactingSemigroup[CMS[Bytes]](10000)
  ...
  /**
   * Like a hashJoin, this joiner does not see all the values V at one time, only one at a time. This is
   * sufficient to implement join and leftJoin
   */
  def cogroup[V2, R](right: TypedPipe[(K, V2)])(
      joiner: (K, V, Iterable[V2]) => Iterator[R]
  ): SketchJoined[K, V, V2, R] =
    new SketchJoined(this, right, numReducers)(joiner)

  /**
   * Does a logical inner join but replicates the heavy keys of the left hand side across the reducers
   */
  def join[V2](right: TypedPipe[(K, V2)]): SketchJoined[K, V, V2, (V, V2)] =
    cogroup(right)(Joiner.hashInner2)

  /**
   * Does a logical left join but replicates the heavy keys of the left hand side across the reducers
   */
  ...
  private implicit def intKeyOrd: Ordering[(Int, K)] = {
    ...
    kord match {
      case kos: OrderedSerialization[_] =>
        new OrderedSerialization2(ordSer[Int], kos.asInstanceOf[OrderedSerialization[K]])
...
object SketchJoined {
  implicit def toTypedPipe[K, V, V2, R](joined: SketchJoined[K, V, V2, R]): TypedPipe[(K, R)] =
    joined.toTypedPipe
