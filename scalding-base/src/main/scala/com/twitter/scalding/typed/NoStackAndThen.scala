...
/**
 * This type is used to implement .andThen on a function in a way that will never blow up the stack. This is
 * done to prevent deep scalding TypedPipe pipelines from blowing the stack
 *
...
object NoStackAndThen {
  ...
  private final case class NonEmpty[-A, B, +C](head: A => B, rest: ReversedStack[B, C])
      extends ReversedStack[A, C]

  private[scalding] final case class WithStackTrace[A, B](
  ...
  private final case class NoStackMore[A, B, C](first: NoStackAndThen[A, B], andThenFn: (B) => C)
