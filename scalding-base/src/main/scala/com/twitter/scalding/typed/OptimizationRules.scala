...
object OptimizationRules {
  ...
  import Literal.{Binary, Unary}
  import TypedPipe._

  /**
   * Since our TypedPipe is covariant, but the Literal is not this is actually safe in this context, but not
   * in general
   */
  ...
  private def handleReduceStep[K, V1, V2](
  ...
  private def handleHashCoGroup[K, V, V2, R](
      hj: HashCoGroup[K, V, V2, R],
      recurse: FunctionK[TypedPipe, LiteralPipe]
  ): LiteralPipe[(K, R)] = {
  ...
  /**
   * Unroll a set of merges up to the first non-merge node, dropping an EmptyTypedPipe from the list
   */
  ...
  /**
   * Make sure each returned item is unique. Any duplicates are merged using flatMap(Iterator.fill(size)(_))
   *
   * TODO: this could be more precise by combining more complex mapping operations into one large flatMap
   */
  ...
  /**
   * It is easier for planning if all fanouts are made explicit. This rule adds a Fork node every time there
   * is a fanout
   *
   * This rule applied first makes it easier to match in subsequent rules without constantly checking for
   * fanout nodes.
   *
   * This can increase the number of map-reduce steps compared to simply recomputing on both sides of a fork
   */
  object AddExplicitForks extends Rule[TypedPipe] {
    ...
    /**
     * The casts in here are safe, but scala loses track of the types in these kinds of pattern matches. We
     * can fix it by changing the types on the identity reduces to use EqTypes[V1, V2] in case class and
     * leaving the V2 parameter.
     */
    private def forkReduceStep[A, B, C](
        on: Dag[TypedPipe],
        rs: ReduceStep[A, B, C]
    ): Option[ReduceStep[A, B, C]] =
      maybeFork(on, rs.mapped).map(ReduceStep.setInput(rs, _))

    private def forkHashJoinable[K, V](
        on: Dag[TypedPipe],
        hj: HashJoinable[K, V]
    ): Option[HashJoinable[K, V]] =
      hj match {
        case step @ IdentityReduce(_, _, _, _, _) =>
   ...
   *
   * also if a filterKeys follows a filter, we might as well compose because we can't push the filterKeys up
   * higher
   */
  ...
  /**
   * If we assume that Orderings are coherent, which we do generally in scalding in joins for instance, we can
   * compose two reduce steps
   */
  ...
  object ComposeWithOnComplete extends PartialRule[TypedPipe] {
    def applyWhere[T](on: Dag[TypedPipe]) = { case WithOnComplete(WithOnComplete(pipe, fn0), fn1) =>
   ...
   *
   * This is a rule you may want to apply after having composed all the maps first
   */
   ...
   *
   * This is a rule you may want to apply after having composed all the filters first
   */
   ...
   * This is a rule you may want to apply after having composed all the filters first
   *
   * This may be a deoptimization on some platforms that have native filters since you could avoid the
   * Iterator boxing in that case.
   */
        ...
        filter.input match {
          case fm: Mapped[a, b] =>
            Some(
              FlatMapped[a, b](
                fm.input,
                FlatMappedFn.fromMap(fm.fn).combine(FlatMappedFn.fromFilter(filter.fn))
              )
            )
          case _ => None
        }
  ...
  /**
   * This rule is important in that it allows us to reduce the number of nodes in the graph, which is helpful
   * to speed up rule application
   */
  object ComposeDescriptions extends PartialRule[TypedPipe] {
    def combine(descs1: List[(String, Boolean)], descs2: List[(String, Boolean)]): List[(String, Boolean)] = {
      ...
      combined
        .foldLeft((Set.empty[String], List.empty[(String, Boolean)])) {
  ...
  /**
   * In scalding 0.17 and earlier, descriptions were automatically pushdown below merges and
   * flatMaps/map/etc..
   */
  ...
  /**
   * (a ++ a) == a.flatMap { t => List(t, t) } This is a very simple rule that is subsumed by DeDiamondMappers
   * below
   */
  ...
  /**
   * This is a more expensive, but more general version of the previous rule: we can merge trailing mapping
   * operations that originate at a common node.
   *
   * After this rule, the only diamonds that exist have at least one non-mapping operation on the path.
   */
  object DeDiamondMappers extends Rule[TypedPipe] {
    sealed abstract class Mapper[+A] {
      ...
      def withDescriptions(desc: List[(String, Boolean)]): Mapper.Aux[Init, A] =
        Mapper(input, fn, ComposeDescriptions.combine(descriptions, desc))
    ...
    object Mapper {
      ...
      def apply[A, B](
          p: TypedPipe[A],
          fn0: FlatMappedFn[A, B],
          desc: List[(String, Boolean)]
      ): Mapper[B] { type Init = A } =
        new Mapper[B] {
          type Init = A
          val input = p
          val fn = fn0
  ...
  /**
   * After a forceToDisk there is no need to immediately fork. Calling forceToDisk twice in a row is the same
   * as once. Calling fork twice in a row is the same as once.
   */
  ...
  /**
   * If a fork has no fan-out when planned, it serves no purpose and is safe to remove. Likewise, there is no
   * reason to put a forceToDisk immediatle after a source
   */
   ...
   *
   * This is arguably not a great idea, but scalding has always done it to minimize accidental map-reduce
   * steps
   */
   ...
   *   1. the final write 2. at the point we are doing a shuffle anyway.
   *
   * By defering merge as long as possible, we hope to find more such cases
   */
  ...
  /**
   * Push filterKeys up as early as possible. This can happen before a shuffle, which can be a major win. This
   * allows you to write generic methods that return all the data, but if downstream someone only wants
   * certain keys they don't pay to compute everything.
   *
   * This is an optimization we didn't do in scalding 0.17 and earlier because .toTypedPipe on the group
   * totally hid the structure from us
   */
  object FilterKeysEarly extends Rule[TypedPipe] {
    private def filterReduceStep[K, V1, V2](
        rs: ReduceStep[K, V1, V2],
        fn: K => Boolean
    ): ReduceStep[K, V1, V2] =
    ...
    private def filterCoGroupable[K, V](rs: CoGroupable[K, V], fn: K => Boolean): CoGroupable[K, V] =
      rs match {
        case rs: ReduceStep[K @unchecked, v1, V @unchecked] =>
          ReduceStep
            .toHashJoinable(filterReduceStep(rs, fn))
            .getOrElse {
              sys.error(
                "unreachable: filterReduceStep returns the same type, and this input type was CoGroupable"
              )
            }
  ...
  /**
   * EmptyTypedPipe is kind of zero of most of these operations We go ahead and simplify as much as possible
   * if we see an EmptyTypedPipe
   */
  ...
  /**
   * This is useful on map-reduce like systems to avoid serializing data into the system that you are going to
   * then filter
   */
  ...
  /**
   * ForceToDisk before hashJoin, this makes sure any filters have been applied
   */
    ...
    final def maybeForce[T](t: TypedPipe[T]): TypedPipe[T] =
      t match {
        ...
        case WithOnComplete(
              pipe,
              fn
            ) => // TODO it is not clear this is safe in cascading 3, since oncomplete is an each
  ...
  /**
   * Prefer to do mapValues/flatMapValues in a Reduce/Join so we can avoid some boxing in-and-out of cascading
   */
  object MapValuesInReducers extends PartialRule[TypedPipe] {
    ...
    def applyWhere[T](on: Dag[TypedPipe]) = {
      ...
      case f @ Filter(_, _) if handleFilter(f).isDefined =>
        handleFilter(f).getOrElse(sys.error("unreachable: already checked isDefined"))
  ...
  /**
   * Like kinds can be composed .map(f).map(g), filter(f).filter(g) etc...
   */
  val composeSame: Rule[TypedPipe] =
    Rule.orElse(List(ComposeMap, ComposeFilter, ComposeFlatMap, ComposeWithOnComplete))

  /**
   * If you are going to do a flatMap, following it or preceding it with map/filter you might as well compose
   * into the flatMap
   */
  val composeIntoFlatMap: Rule[TypedPipe] =
    Rule.orElse(List(ComposeMapFlatMap, ComposeFilterFlatMap, ComposeFlatMap))

  val simplifyEmpty: Rule[TypedPipe] =
    EmptyIsOftenNoOp.orElse(EmptyIterableIsEmpty)

  /**
   * These are a list of rules to be applied in order (Dag.applySeq) that should generally always improve
   * things on Map/Reduce-like platforms.
   *
   * These are rules we should apply to any TypedPipe before handing to cascading. These should be a bit
   * conservative in that they should be highly likely to improve the graph.
   */
