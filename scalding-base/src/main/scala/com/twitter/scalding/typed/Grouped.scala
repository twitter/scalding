...
object CoGroupable extends Serializable {
  ...
  /**
   * Return true if there is a sum occurring at the end the mapGroup transformations If we know this is
   * finally summed, we can make some different optimization choices
   *
  ...
  /**
   * Returns true if the group mapping function definitely returns 0 or 1 element.
   *
  ...
  /**
   * Returns true if the group mapping function does not increase the number of items in the Iterator
   */
  final def atMostInputSizeFn[A, B, C](fn: (A, Iterator[B]) => Iterator[C]): Boolean =
    fn match {
      ...
      case EmptyGuard(fn) if atMostOneFn(fn) =>
        true // since 0 always goes to 0 due to empty guard, and 1 -> 0 or 1 since atMostOne
...
sealed trait CoGroupable[K, +R] extends HasReducers with HasDescription with Serializable {
  ...
  /**
   * This function is not type-safe for others to call, but it should never have an error. By construction, we
   * never call it with incorrect types. It would be preferable to have stronger type safety here, but unclear
   * how to achieve, and since it is an internal function, not clear it would actually help anyone for it to
   * be type-safe
   */
   ...
   *
   * Note that from the type signature we see that the right side is iterated (or may be) over and over, but
   * the left side is not. That means that you want the side with fewer values per key on the right. If both
   * sides are similar, no need to worry. If one side is a one-to-one mapping, that should be the "smaller"
   * side.
   */
  def cogroup[R1, R2](smaller: CoGroupable[K, R1])(
      fn: (K, Iterator[R], Iterable[R1]) => Iterator[R2]
  ): CoGrouped[K, R2] =
...
object CoGrouped extends Serializable {
  ...
  final case class Pair[K, A, B, C](
    ...
    /**
     * Avoid capturing anything below as it will need to be serialized and sent to all the reducers.
     */
    def joinFunction = {

      /**
       * if there is at most one value on the smaller side definitely cache the result to avoid repeatedly
       * computing it
       */
  ...
  final case class WithDescription[K, V](on: CoGrouped[K, V], description: String) extends CoGrouped[K, V] {
  ...
  final case class MapGroup[K, V1, V2](on: CoGrouped[K, V1], fn: (K, Iterator[V1]) => Iterator[V2])
      extends CoGrouped[K, V2] {
    def inputs = on.inputs
    def reducers = on.reducers
    def descriptions: Seq[String] = on.descriptions
    def keyOrdering = on.keyOrdering
    def joinFunction =
      MultiJoinFunction.MapGroup(on.joinFunction, fn)
  }
}

sealed trait CoGrouped[K, +R]
  ...
  /**
   * It seems complex to push a take up to the mappers before a general join. For some cases (inner join), we
   * could take at most n from each TypedPipe, but it is not clear how to generalize that for general
   * cogrouping functions. For now, just do a normal take.
   */
 ...
 * i.e., HashJoinable is a strict subset of CoGroupable (CoGrouped, for instance is CoGroupable, but not
 * HashJoinable).
 */
...
/**
 * This encodes the rules that 1) sorting is only possible before doing any reduce, 2) reversing is only
 * possible after sorting. 3) unsorted Groups can be CoGrouped or HashJoined
 *
...
/**
 * After sorting, we are no longer CoGroupable, and we can only call reverse in the initial SortedGrouped
 * created from the Sortable: .sortBy(_._2).reverse for instance
 *
...
/**
 * This is the state after we have done some reducing. It is not possible to sort at this phase, but it is
 * possible to do a CoGrouping or a HashJoin.
 */
...
/**
 * All sorting methods defined here trigger Hadoop secondary sort on key + value. Hadoop secondary sort is
 * external sorting. i.e. it won't materialize all values of each key in memory on the reducer.
 */
...
/**
 * This is a class that models the logical portion of the reduce step. details like where this occurs, the
 * number of reducers, etc... are left in the Grouped class
 */
...
object ReduceStep extends Serializable {
  ...
  /**
   * assuming coherent Orderings on the A, in some cases ReduceSteps can be combined. Note: we have always
   * assumed coherant orderings in scalding with joins where both sides have their own Ordering, so we argue
   * this is not different.
   *
   * If a user has incoherant Orderings, which are already dangerous, they can use .forceToDisk between reduce
   * steps, however, a better strategy is to use different key types.
   *
   * The only case where they can't is when there are two different value sorts going on.
   */
  def maybeCompose[A, B, C, D](
      rs1: ReduceStep[A, B, C],
      rs2: ReduceStep[A, C, D]
  ): Option[ReduceStep[A, B, D]] = {
  ...
  def mapGroup[A, B, C, D](
      rs: ReduceStep[A, B, C]
  )(fn: (A, Iterator[C]) => Iterator[D]): ReduceStep[A, B, D] =
    rs match {
      case step @ IdentityReduce(_, _, _, _, _) =>
        type Res[T] = ReduceStep[A, T, D]
...
final case class IdentityReduce[K, V1, V2](
   ...
   * This is a big help if there are relatively few keys and n is relatively small.
   */
  ...
  override def sum[U >: V2](implicit sg: Semigroup[U]) = {
    ...
    UnsortedIdentityReduce[K, U, U](
      keyOrdering,
      upipe.sumByLocalKeys,
      reducers,
      descriptions,
      implicitly
    ).sumLeft
...
final case class UnsortedIdentityReduce[K, V1, V2](
  ...
  override def bufferedTake(n: Int) =
    if (n < 1) {
      ...
      UnsortedIdentityReduce[K, V1, V2](
        keyOrdering,
        pretake,
        reducers,
        descriptions,
...
final case class IdentityValueSortedReduce[K, V1, V2](
  ...
  override def reverse: IdentityValueSortedReduce[K, V1, V2] =
    IdentityValueSortedReduce[K, V1, V2](
      keyOrdering,
      mapped,
      valueSort.reverse,
      reducers,
  ...
  override def mapGroup[V3](fn: (K, Iterator[V2]) => Iterator[V3]) = {
    ...
    type TK[V] = TypedPipe[(K, V)]
    ValueSortedReduce[K, V2, V3](
      keyOrdering,
      evidence.subst[TK](mapped),
      evidence.subst[Ordering](valueSort),
      gfn,
      reducers,
      descriptions
    )
  ...
  /**
   * We are sorting then taking. Optimized for small take values If we take <= 1, we use an in-memory-based
   * method. To force a memory-based take, use bufferedTake Otherwise, we send all the values to the reducers
   */
...
final case class ValueSortedReduce[K, V1, V2](
  ...
  override def withReducers(red: Int) =
    ...
    ValueSortedReduce[K, V1, V2](keyOrdering, mapped, valueSort, reduceFn, Some(red), descriptions)
  ...
  override def mapGroup[V3](fn: (K, Iterator[V2]) => Iterator[V3]) = {
    ...
    ValueSortedReduce[K, V1, V3](keyOrdering, mapped, valueSort, newReduce, reducers, descriptions)
  }
}
