 ...
 * which it evaluates matrices, and replaces equivalent terms to avoid recomputation. Also, this code puts the
 * parenthesis in the optimal place in terms of size according to the sizeHints. For instance: (A*B)*C ==
 * A*(B*C) but if B is a 10 x 10^6 matrix, and C is 10^6 x 100, it is better to do the B*C product first in
 * order to avoid storing as much intermediate output.
 *
 * NOTE THIS REQUIREMENT: for each formula, you can only have one Ring[V] in scope. If you evaluate part of
 * the formula with one Ring, and another part with another, you must go through a TypedPipe (call
 * toTypedPipe) or the result may not be correct.
 */
sealed trait Matrix2[R, C, V] extends Serializable {
   ...
   */
  def #*#(that: Matrix2[R, C, V])(implicit ring: Ring[V]): Matrix2[R, C, V] =
  ...
  /**
   * Users should never need this. This is the current Matrix2, but in most optimized form. Usually, you will
   * just do matrix operations until you eventually call write or toTypedPipe
   */
  ...
  /**
   * the result is the same as considering everything on the this to be like a 1 value so we just sum, using
   * only a monoid on VecV, where this Matrix has the value true. This is useful for graph propagation of
   * monoids, such as sketchs like HyperLogLog, BloomFilters or CountMinSketch. TODO This is a special kind of
   * product that could be optimized like Product is
   */
  ...
  def propagateRow[C2](
      mat: Matrix2[C, C2, Boolean]
  )(implicit ev: =:=[R, Unit], mon: Monoid[V], mj: MatrixJoiner2): Matrix2[Unit, C2, V] =
  ...
  def rowL2Normalize(implicit num: Numeric[V], mj: MatrixJoiner2): Matrix2[R, C, Double] = {
    val matD =
      MatrixLiteral(this.toTypedPipe.map { case (r, c, x) => (r, c, num.toDouble(x)) }, this.sizeHint)
    lazy val result = MatrixLiteral(
      this.toTypedPipe.map { case (r, c, x) => (r, c, num.toDouble(x) * num.toDouble(x)) },
      this.sizeHint
    ).sumColVectors.toTypedPipe
  ...
  def rowL1Normalize(implicit num: Numeric[V], mj: MatrixJoiner2): Matrix2[R, C, Double] = {
    val matD =
      MatrixLiteral(this.toTypedPipe.map { case (r, c, x) => (r, c, num.toDouble(x).abs) }, this.sizeHint)
  ...
  /**
   * Consider this Matrix as the r2 row of a matrix. The current matrix must be a row, which is to say, its
   * row type must be Unit.
   */
...
/**
 * This trait allows users to plug in join algorithms where they are needed to improve products and
 * propagations. The default works well in most cases, but highly skewed matrices may need some special
 * handling
 */
...
object MatrixJoiner2 {
  ...
  def join[R, C, V, C2, V2](left: Matrix2[R, C, V], right: Matrix2[C, C2, V2])(implicit
      mj: MatrixJoiner2
  ): TypedPipe[(C, ((R, V), (C2, V2)))] =
    mj.join(left, right)
}
...
/**
 * This uses standard join if the matrices are comparable size and large, otherwise, if one is much smaller
 * than the other, we use a hash join
 */
class DefaultMatrixJoiner(sizeRatioThreshold: Long) extends MatrixJoiner2 {
  def join[R, C, V, C2, V2](
    ...
    def swapInner[M, N](t: TypedPipe[(C, (M, N))]): TypedPipe[(C, (N, M))] = t.mapValues { t: (M, N) =>
      t.swap
    }
    // TODO:
...
final case class OneC[R, V](implicit override val rowOrd: Ordering[R]) extends Matrix2[R, Unit, V] {
  ...
  def transpose = OneR()
  override def negate(implicit g: Group[V]) =
    sys.error("Only used in intermediate computations, try (-1 * OneC)")
...
final case class OneR[C, V](implicit override val colOrd: Ordering[C]) extends Matrix2[Unit, C, V] {
  ...
  def transpose = OneC()
  override def negate(implicit g: Group[V]) =
    sys.error("Only used in intermediate computations, try (-1 * OneR)")
  def toTypedPipe = sys.error("Only used in intermediate computations")
}
 ...
 * @param ring
 * @param expressions
 *   a HashMap of common subtrees; None if possibly not optimal (did not go through optimize), Some(...) with
 *   a HashMap that was created in optimize
 */
final case class Product[R, C, C2, V](
    left: Matrix2[R, C, V],
    right: Matrix2[C, C2, V],
    ring: Ring[V],
    expressions: Option[Map[Matrix2[R, C2, V], TypedPipe[(R, C2, V)]]] = None
)(implicit val joiner: MatrixJoiner2)
    extends Matrix2[R, C2, V] {

  /**
   * Structural, NOT mathematical equality (e.g. (A*B) * C != A * (B*C)) Used for the Matrix2OptimizationTest
   * (so that it doesn't care about expressions)
   */
...
final case class Sum[R, C, V](left: Matrix2[R, C, V], right: Matrix2[R, C, V], mon: Monoid[V])
...
final case class HadamardProduct[R, C, V](left: Matrix2[R, C, V], right: Matrix2[R, C, V], ring: Ring[V])
    extends Matrix2[R, C, V] {
...
)(implicit override val rowOrd: Ordering[R], override val colOrd: Ordering[C])
    extends Matrix2[R, C, V] {
...
object Matrix2 {
  ...
  /**
   * The original prototype that employs the standard O(n^3) dynamic programming procedure to optimize a
   * matrix chain factorization.
   *
   * Now, it also "prefers" more spread out / bushy / less deep factorization which reflects more the
   * Map/Reduce nature.
   */
   ...
   * i.e. matrix product chains that are not interrupted by summations. One example: A*B*C*(D+E)*(F*G) =>
   * "basic blocks" are ABC, D, E, and FG
   *
   * + it now does "global" optimization - i.e. over optimize over basic blocks. In the above example, we'd
   * treat (D+E) as a temporary matrix T and optimize the whole chain ABCTFG
   *
   * Not sure if making use of distributivity to generate more variants would be good. In the above example,
   * we could also generate ABCDFG + ABCEFG and have basic blocks: ABCDFG, and ABCEFG. But this would be
   * almost twice as much work with the current cost estimation.
   */
  def optimize[V](mf: Matrix2[Any, Any, V]): (BigInt, Matrix2[Any, Any, V]) = {
    ...
    def optimizeBasicBlocks(
        mf: Matrix2[Any, Any, V]
    ): (List[Matrix2[Any, Any, V]], BigInt, Option[Ring[V]], Option[MatrixJoiner2]) =
      mf match {
        ...
        case Sum(left, right, mon) => {
          ...
          val (cost1, newLeft) =
            optimizeProductChain(lastLChain.toIndexedSeq, pair(ringL, joinerL)) // linter:ignore
          val (cost2, newRight) =
            optimizeProductChain(lastRChain.toIndexedSeq, pair(ringR, joinerR)) // linter:ignore
          (
    ...
    val (potentialCost, finalResult) =
      optimizeProductChain(lastChain.toIndexedSeq, pair(ring, joiner)) // linter:ignore
