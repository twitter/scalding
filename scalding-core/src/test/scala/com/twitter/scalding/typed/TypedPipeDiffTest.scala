...
class TypedPipeDiffTest extends FunSuite {
  ...
  val rightArr =
    List(Array[Byte](2, 2, 2), Array[Byte](2, 2, 2), Array[Byte](3, 3, 5, 3, 2), Array[Byte](0, 1, 1))
  ...
  test("diffArrayPipesWithoutOrdering works for arrays of objects with no ordering") {
    ...
    assert(expectedSortedArrDiff === sort(diff.inMemoryToList.map { case (arr, counts) =>
...
object TypedPipeDiffLaws {
  ...
  def diffArrayLaw[T](implicit arb: Arbitrary[List[Array[T]]], ct: ClassTag[T]): Prop = Prop.forAll {
