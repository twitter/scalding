...
class TypedPipeDiffTest extends FunSuite {
  ...
  val leftArr = List(Array[Byte](3, 3, 5, 3, 2), Array[Byte](2, 2, 2), Array[Byte](0, 1, 0))

  val rightArr = List(Array[Byte](2, 2, 2),
                      Array[Byte](2, 2, 2),
                      Array[Byte](3, 3, 5, 3, 2),
                      Array[Byte](0, 1, 1))

  val expectedSortedArrDiff = List((Array[Byte](0, 1, 0).toSeq, (1, 0)),
  ...
  test("diffArrayPipes works for arrays") {
    ...
    val diff =
      TypedPipeDiff.diffArrayPipes(pipe1, pipe2).map { case (arr, counts) => (arr.toSeq, counts) }
  ...
  test("diffWithoutOrdering does not require ordering") {
    ...
    assert(expectedSortedDiff === diff.inMemoryToList.map {
  ...
  test("diffArrayPipesWithoutOrdering works for arrays of objects with no ordering") {
    ...
    assert(expectedSortedArrDiff === sort(diff.inMemoryToList.map {
...
object TypedPipeDiffLaws {
  ...
  def checkDiff[T](left: List[T], right: List[T], diff: List[(T, (Long, Long))]): Boolean = {
    ...
    val expected = MapAlgebra
      .sumByKey(left.map((_, (1L, 0L))).iterator ++ right.map((_, (0L, 1L))).iterator)
      .filter { case (t, (rCount, lCount)) => rCount != lCount }

    noDuplicates && expected == diff.toMap
  }

  def checkArrayDiff[T](left: List[Array[T]],
                        right: List[Array[T]],
  ...
  def diffArrayLaw[T](implicit arb: Arbitrary[List[Array[T]]], ct: ClassTag[T]): Prop =
