...
class TypedApiTest extends WordSpec with Matchers with TBddDsl {
  ...
  "A test with a two sources" should {
    ...
    "accept an operation from two tuple-typed pipes" in {
      ...
      } When { (gender: TypedPipe[(String, String)], age: TypedPipe[(String, Int)]) =>
        gender.group
          .join(age.group)
          .toTypedPipe
      ...
      } Then { buffer: mutable.Buffer[(String, String, Int)] =>
    ...
    "accept an operation from two case classes-typed pipes" in {
      ...
      } When { (gender: TypedPipe[UserWithGender], age: TypedPipe[UserWithAge]) =>
        gender
          .groupBy(_.name)
