...
class MacrosUnitTests extends WordSpec with Matchers {
  ...
  def isMacroTupleConverterAvailable[T](
      implicit proof: TupleConverter[T] = dummy.asInstanceOf[TupleConverter[T]]) =
    proof.isInstanceOf[MacroGenerated]

  def isMacroTypeDescriptorAvailable[T](
      implicit proof: TypeDescriptor[T] = dummy2.asInstanceOf[TypeDescriptor[T]]) =
    proof.isInstanceOf[MacroGenerated]
  ...
  def shouldRoundTripOther[T: IsCaseClass: TupleSetter: TupleConverter](te: TupleEntry,
                                                                        t: T): Unit = {
    val inter = mgConv(te)
    inter shouldBe t
  ...
  "MacroGenerated TupleSetter and TupleConverter" should {
    "round trip class -> tupleentry -> class" in {
      ...
      shouldRoundTrip(
        SampleClassB(SampleClassA(100, "onehundred"), SampleClassA(-1, "zero"), "what"))
      val a = SampleClassA(73, "hrmA1")
      val b = SampleClassB(a, a, "hrmB1")
      val c = SampleClassC(a,
                           b,
