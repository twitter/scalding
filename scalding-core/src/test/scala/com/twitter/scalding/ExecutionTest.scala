...
object ExecutionTestJobs {
  ...
  def writeExecutionWithTempFile(tempFile: String,
                                 testData: List[String]): Execution[List[String]] = {
...
class ZippedExecutionWithTempFiles(args: Args,
                                   tempFileOne: String,
                                   tempFileTwo: String,
...
class ExecutionTest extends WordSpec with Matchers {
  ...
  "An Execution" should {
    ...
    "Config transformer will isolate Configs" in {
      ...
      doesNotHaveVariable("Should not see variable before we've started transforming")
        ...
        .flatMap(_ =>
          doesNotHaveVariable("Should not see variable in flatMap's after the isolation"))
        .map(_ => true)
    ...
    "Config transformer will interact correctly with the cache when writing" in {
      ...
      val operationTP = (TypedPipe.from(src) ++ TypedPipe.from((1 until 100).toList))
        .writeExecution(sink)
        .getCounters
        .map(_._2.toMap)
  ...
  "Executions" should {
    "shutdown hook should clean up temporary files" in {
      ...
      val cleanupThread = TempFileCleanup(Seq(tempFileOne.toFile.getAbsolutePath,
                                              tempFileTwo.toFile.getAbsolutePath),
                                          mode)
      cleanupThread.run()
    ...
    "clean up temporary files on exit" in {
      ...
      cleanupHook.get
        .asInstanceOf[TempFileCleanup]
        .filesToCleanup should contain theSameElementsAs Set(tempFile)
      cleanupHook.get.run()
    ...
    "clean up temporary files on exit with a zip" in {
      ...
      cleanupHook.get
        .asInstanceOf[TempFileCleanup]
        .filesToCleanup should contain theSameElementsAs Set(tempFileOne, tempFileTwo)
      cleanupHook.get.run()
      // Remove the hook so it doesn't show up in the list of shutdown hooks for other tests
      Runtime.getRuntime.removeShutdownHook(cleanupHook.get)
    }
    ...
    "maintains equality and hashCode after reconstruction" when {
      ...
      val futureF = { _: ConcurrentExecutionContext =>
        Future.successful(10)
      }
      val futureF2 = { _: ConcurrentExecutionContext =>
        Future.successful(10)
      }
      val fnF = { (_: Config, _: Mode) =>
        null
      }
      val fnF2 = { (_: Config, _: Mode) =>
        null
      }
      val withIdF = { _: UniqueID =>
        Execution.unit
      }
      val withIdF2 = { _: UniqueID =>
        Execution.unit
      }
      val mapF = { _: Int =>
        12
      ...
      "Execution#map" in {
        reconstructibleLaws(Execution.fromFuture(futureF).map(mapF),
      ...
      "Execution.sequence" in {
        reconstructibleLaws(
          Execution.sequence(
            Seq(Execution.fromFuture(futureF),
    ...
    "Has consistent hashCode and equality for mutable" when {
      ...
      case class FromFnMutable(var x: Int = 0)
          extends Function2[Config, Mode, Null]
          with MutableX[Unit] {
      ...
      def mutableLaws[T, U <: MutableX[T]](mutableGen: => U,
      ...
      "Execution.fromFuture" in {
        mutableLaws(FromFutureMutable(), Some({ x: Int =>
          x
        }))
      }
      ...
      "Execution.withId" in {
        mutableLaws(WithIdMutable(), Some({ x: Int =>
          x
        }))
      }

      "Execution#map" in {
        mutableLaws(MapMutable(), Some({ x: Int =>
          x * x
        }))
      }

      "Execution#zip" in {
        mutableLaws(ZipMutable(), Some({ x: Int =>
          (x, x + 20)
        }))
      }

      "Execution.sequence" in {
        mutableLaws(SequenceMutable(), Some({ x: Int =>
          Seq(x, x * 3)
        }))
      }
    }
