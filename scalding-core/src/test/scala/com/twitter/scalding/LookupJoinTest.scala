...
class LookupJoinerJob(args: Args) extends Job(args) {
  ...
  LookupJoin(TypedPipe.from(in0).map { case (t, k, v) => (t, (k, v)) }, TypedPipe.from(in1).map {
    case (t, k, v) => (t, (k, v))
  }).map {
      case (t, (k, (v, opt))) =>
...
class LookupJoinedTest extends WordSpec with Matchers {
  ...
  def lookupSumJoin[T: Ordering, K, V, W: Semigroup](in0: Iterable[(T, K, V)],
                                                     in1: Iterable[(T, K, W)]) = {
        ...
        (k,
         v.toList.sorted
           .scanLeft(None: Option[(T, K, W)]) { (old, newer) =>
             old
               .map { case (_, _, w) => (newer._1, newer._2, Semigroup.plus(w, newer._3)) }
               .orElse(Some(newer))
           }
  ...
  "A LookupJoinerJob" should {
    "correctly lookup" in {
      ...
      JobTest(new LookupJoinerJob(_))
        ...
        .sink[(String, String, String, String)](TypedTsv[(String, String, String, String)](
          "output2")) { outBuf =>
...
class WindowLookupJoinerJob(args: Args) extends Job(args) {
  ...
  LookupJoin
    .withWindow(TypedPipe.from(in0).map { case (t, k, v) => (t, (k, v)) },
