...
class Matrix2OptimizationSpec extends WordSpec with Matchers {
  ...
  def literal(tpipe: TypedPipe[(Int, Int, Double)],
              sizeHint: SizeHint): MatrixLiteral[Any, Any, Double] =
    MatrixLiteral(tpipe, sizeHint).asInstanceOf[MatrixLiteral[Any, Any, Double]]
  def product(left: Matrix2[Any, Any, Double],
              right: Matrix2[Any, Any, Double]): Product[Any, Any, Any, Double] =
    Product(left, right, ring)
  def sum(left: Matrix2[Any, Any, Double],
          right: Matrix2[Any, Any, Double]): Sum[Any, Any, Double] = Sum(left, right, ring)
  ...
  val optimizedPlan = product( // linter:ignore
    ...
    product(product(literal(globM, FiniteHint(5, 10)), literal(globM, FiniteHint(10, 20))),
    ...
    product(
      ...
      product(literal(globM, FiniteHint(15, 5)),
              product(literal(globM, FiniteHint(5, 10)),
                      product(literal(globM, FiniteHint(10, 20)),
                              literal(globM, FiniteHint(20, 25)))))
    )
  )
  ...
  val optimizedGlobalPlan = product( // linter:ignore
    product(literal(globM, FiniteHint(30, 35)),
            product(literal(globM, FiniteHint(35, 15)), literal(globM, FiniteHint(15, 5)))),
    product(product(sum(literal(globM, FiniteHint(5, 10)), literal(globM, FiniteHint(5, 10))),
  ...
  val planWithSum = product(
    literal(globM, FiniteHint(30, 35)),
    sum(literal(globM, FiniteHint(35, 25)), literal(globM, FiniteHint(35, 25)))) // linter:ignore
...
object Matrix2Props extends Properties("Matrix2") {
  ...
  def productChainGen(
      ...
      target: Int,
      prevCol: Long,
      result: List[MatrixLiteral[Any, Any, Double]]): List[MatrixLiteral[Any, Any, Double]] =
  ...
  def evaluate(mf: Matrix2[Any, Any, Double]): BigInt = {
     ...
     */
    class LabeledTree(val range: (Int, Int),
                      val left: Option[LabeledTree],
                      val right: Option[LabeledTree]) {
    ...
    def labelTree(p: Matrix2[Any, Any, Double], start: Int): Option[LabeledTree] =
      p match {
        ...
        case Product(left @ Product(_, _, _, _), right @ MatrixLiteral(_, _), _, _) => {
          ...
          Some(
            new LabeledTree((labelLeft.get.range._1, labelLeft.get.range._2 + 1), labelLeft, None))
        }
        case Product(left, right, _, _) => {
          val labelLeft = labelTree(left, start)
          val labelRight = labelTree(right, labelLeft.get.range._2 + 1)
          Some(
            new LabeledTree((labelLeft.get.range._1, labelRight.get.range._2),
                            labelLeft,
                            labelRight))
        }
        case _ => None
