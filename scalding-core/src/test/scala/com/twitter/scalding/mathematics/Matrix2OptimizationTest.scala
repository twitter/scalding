...
/**
 * Unit tests used in development (stronger properties are tested in ScalaCheck tests at the end)
 */
class Matrix2OptimizationSpec extends WordSpec with Matchers {
  ...
  def literal(tpipe: TypedPipe[(Int, Int, Double)], sizeHint: SizeHint): MatrixLiteral[Any, Any, Double] =
    MatrixLiteral(tpipe, sizeHint).asInstanceOf[MatrixLiteral[Any, Any, Double]]
  def product(
      left: Matrix2[Any, Any, Double],
      right: Matrix2[Any, Any, Double]
  ): Product[Any, Any, Any, Double] = Product(left, right, ring)
  def sum(left: Matrix2[Any, Any, Double], right: Matrix2[Any, Any, Double]): Sum[Any, Any, Double] =
    Sum(left, right, ring)
  ...
  val optimizedPlan = product( // linter:ignore
    product(
      ...
      product(literal(globM, FiniteHint(35, 15)), literal(globM, FiniteHint(15, 5)))
    ),
    product(
      product(literal(globM, FiniteHint(5, 10)), literal(globM, FiniteHint(10, 20))),
  ...
  val unoptimizedPlan = product(
    literal(globM, FiniteHint(30, 35)), // linter:ignore
    product(
  ...
  val productSequence = IndexedSeq(
    literal(globM, FiniteHint(30, 35)),
    literal(globM, FiniteHint(35, 15)),
    literal(globM, FiniteHint(15, 5)),
    literal(globM, FiniteHint(5, 10)),
  ...
  val combinedSequence = List(
    ...
    IndexedSeq(literal(globM, FiniteHint(30, 35)), literal(globM, FiniteHint(35, 25)))
  )

  val planWithSum = product(
    literal(globM, FiniteHint(30, 35)),
    sum(literal(globM, FiniteHint(35, 25)), literal(globM, FiniteHint(35, 25)))
  ) // linter:ignore
  ...
  val optimizedGraphVectorPlan = product( // linter:ignore
    ...
    product(
      ...
      product(
        literal(globM, FiniteHint(30, 30)),
        product(literal(globM, FiniteHint(30, 30)), literal(globM, FiniteHint(Long.MaxValue, 1)))
      )
    )
  )
...
object Matrix2Props extends Properties("Matrix2") {
  ...
  /**
   * Function that recursively estimates a cost of a given MatrixFormula / plan. This is the used in the tests
   * for checking whether an optimized plan has a cost <= a randomized plan. The cost estimation of this
   * evaluation should return the same values as the one used in building optimized plans -- this is checked
   * in the tests below.
   * @return
   *   resulting cost
   */
  def evaluate(mf: Matrix2[Any, Any, Double]): BigInt = {
    ...
    /**
     * This function strips off the formula into a list of independent product chains (i.e. same as
     * matrixFormulaToChains in Prototype, but has Products instead of IndexedSeq[Literal])
     */
    def toProducts(
        mf: Matrix2[Any, Any, Double]
    ): (Option[Product[Any, Any, Any, Double]], List[Product[Any, Any, Any, Double]]) =
      mf match {
        ...
        case Product(left, right, _, _) => {
          ...
          if (lastLP.isDefined && lastRP.isDefined) {
            ...
            val newP =
              if (lastLP.isDefined) List(lastLP.get) else if (lastRP.isDefined) List(lastRP.get) else Nil
    ...
    /**
     * This function evaluates a product chain in the same way as the dynamic programming procedure computes
     * cost (optimizeProductChain - computeCosts in Prototype)
     */
    def evaluateProduct(
      ...
      p match {
        case Product(left @ MatrixLiteral(_, _), right @ MatrixLiteral(_, _), _, _) => {
          ...
          Some((left.sizeHint * (left.sizeHint * right.sizeHint)).total.get, left, right)
        }
        case Product(left @ MatrixLiteral(_, _), right @ Product(_, _, _, _), _, _) => {
  ...
  /**
   * Verifying "evaluate" function - that it does return the same overall costs as what is estimated in the
  ...
  /**
   * "Proof": the goal property that estimated costs of optimized plans or product chains are less than or
   * equal to costs of randomized equivalent plans or product chains
   */
  property("a cost of an optimized chain of matrix products is <= a random one") = forAll {
