...
object RichPipeSpecification extends Properties("RichPipe") {
  ...
  val smallNames = Gen.choose(0, 12) flatMap { sz =>
    Gen.listOfN(sz, alphaChar)
  } map (_.mkString)

  val longNames = Gen.choose(13, 256) flatMap { sz =>
    Gen.listOfN(sz, alphaChar)
  } map (_.mkString)

  property(
    "assignName carries over the whole old name "
      + "if it's 12 characters or less") = forAll(smallNames) { (name: String) =>
    val basePipeNumber = extractPipeNumber(RichPipe.getNextName)
  ...
  property(
    "assignName carries over the last 12 characters of the old name "
      + "if it's more than 12 characters") = forAll(longNames) { (name: String) =>
    val basePipeNumber = extractPipeNumber(RichPipe.getNextName)
