...
class BlockJoinPipeTest extends WordSpec with Matchers {
  "An InnerProductJob" should {
    ...
    def runJobWithArguments(left: Int = 1, right: Int = 1, joiner: String = "i")(
        callback: Buffer[(Int, Int, Double)] => Unit): Unit =
    ...
    "throw an exception when used with an invalid LeftJoin" in {
      an[InvalidJoinModeException] should be thrownBy runJobWithArguments(joiner = "l", left = 2) {
        _ =>
        }
    }

    "throw an exception when used with an invalid RightJoin" in {
      an[InvalidJoinModeException] should be thrownBy runJobWithArguments(joiner = "r", right = 2) {
        _ =>
        }
    }
  }
