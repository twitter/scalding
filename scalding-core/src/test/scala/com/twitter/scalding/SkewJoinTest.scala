...
class SkewJoinJob(args: Args) extends Job(args) {
  ...
  val in0 = Tsv("input0").read.mapTo((0, 1, 2) -> ('x1, 'y1, 's1)) { input: (Int, Int, Int) =>
    input
  }
  val in1 = Tsv("input1").read.mapTo((0, 1, 2) -> ('x2, 'y2, 's2)) { input: (Int, Int, Int) =>
    input
  }
...
class SkewJoinPipeTest extends WordSpec with Matchers {
  ...
  "A SkewInnerProductJob" should {
    "compute skew join with sampleRate = 0.001, using strategy A" in {
      val (sk, inner) =
        runJobWithArguments(new SkewJoinJob(_), sampleRate = 0.001, replicator = "a")
      sk shouldBe inner
    }

    "compute skew join with sampleRate = 0.001, using strategy B" in {
      val (sk, inner) =
        runJobWithArguments(new SkewJoinJob(_), sampleRate = 0.001, replicator = "b")
      sk shouldBe inner
    }
    ...
    "compute skew join with replication factor 5, using strategy A" in {
      val (sk, inner) =
        runJobWithArguments(new SkewJoinJob(_), replicationFactor = 5, replicator = "a")
      sk shouldBe inner
    }
...
class CollidingKeySkewJoinJob(args: Args) extends Job(args) {
  ...
  val in0 = Tsv("input0").read.mapTo((0, 1, 2) -> ('k1, 'k3, 'v1)) { input: (Int, Int, Int) =>
    input
  }
  val in1 = Tsv("input1").read.mapTo((0, 1, 2) -> ('k2, 'k3, 'v2)) { input: (Int, Int, Int) =>
    input
  }
