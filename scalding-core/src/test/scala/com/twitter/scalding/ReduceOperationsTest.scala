      ...
      .groupBy('key) {
        _.sortWithTake[(Long, Double)]((('item_id, 'score), 'top_items), 5) {
          (item_0: (Long, Double), item_1: (Long, Double)) =>
            if (item_0._2 == item_1._2) { item_0._1 > item_1._1 } else { item_0._2 > item_1._2 }
        }
      }
    ...
    JobTest(new SortWithTakeJob(_))
      ...
      .sink[(String, List[(Long, Double)])](Tsv("output0")) { buf =>
        "grouped list" in {
          ...
          whatWeGet
            .get("a")
            .getOrElse("apples") shouldBe (whatWeWant.get("a").getOrElse("oranges"))
          whatWeGet
            .get("b")
            .getOrElse("apples") shouldBe (whatWeWant.get("b").getOrElse("oranges"))
        }
      }
      .runHadoop
      .finish()
    ...
    JobTest(new ApproximateUniqueCountJob(_))
      ...
      .sink[(String, Long)](Tsv("output0")) { buf =>
        "grouped OS count" in {
          ...
          whatWeGet
            .get("laptop")
            .getOrElse("apples") shouldBe (whatWeWant.get("laptop").getOrElse("oranges"))
          whatWeGet
            .get("mobile")
            .getOrElse("apples") shouldBe (whatWeWant.get("mobile").getOrElse("oranges"))
        }
      }
      .runHadoop
      .finish()
