...
class MatrixPipeExtensions(pipe: Pipe) {
  def toMatrix[RowT, ColT, ValT](fields: Fields)(implicit conv: TupleConverter[(RowT, ColT, ValT)],
                                                 setter: TupleSetter[(RowT, ColT, ValT)]) = {
    val matPipe = RichPipe(pipe).mapTo(fields -> ('row, 'col, 'val))((tup: (RowT, ColT, ValT)) =>
      tup)(conv, setter)
  ...
  def flatMapToMatrix[T, RowT, ColT, ValT](fields: Fields)(
      flatMapfn: T => Iterable[(RowT, ColT, ValT)])(implicit conv: TupleConverter[T],
                                                    setter: TupleSetter[(RowT, ColT, ValT)]) = {
    ...
    pipe
      .groupBy('group, 'row) {
        _.mapReduceMap[(ColT, ValT), Map[ColT, ValT], Map[ColT, ValT]](('col, 'val) -> 'val) {
          (colval: (ColT, ValT)) =>
            Map(colval._1 -> colval._2)
        } { (l: Map[ColT, ValT], r: Map[ColT, ValT]) =>
          l ++ r
        } { (red: Map[ColT, ValT]) =>
          red
        }
  ...
  def toBlockMatrix[GroupT, RowT, ColT, ValT](fields: Fields)(
    ...
    val matPipe = RichPipe(pipe)
      .mapTo(fields -> ('group, 'row, 'col, 'val))((tup: (GroupT, RowT, ColT, ValT)) => tup)(
        conv,
        setter)
  ...
  def mapToBlockMatrix[T, GroupT, RowT, ColT, ValT](fields: Fields)(
      mapfn: T => (GroupT, RowT, ColT, ValT))(implicit conv: TupleConverter[T],
                                              setter: TupleSetter[(GroupT, RowT, ColT, ValT)]) = {
    val matPipe = RichPipe(pipe)
  ...
  def flatMapToBlockMatrix[T, GroupT, RowT, ColT, ValT](fields: Fields)(
      ...
      setter: TupleSetter[(GroupT, RowT, ColT, ValT)]) = {
    val matPipe =
      RichPipe(pipe).flatMapTo(fields -> ('group, 'row, 'col, 'val))(flatMapfn)(conv, setter)
    new BlockMatrix[GroupT, RowT, ColT, ValT](
      new Matrix('row, 'col, 'val, groupPipeIntoMap(matPipe)))
  }

  def toColVector[RowT, ValT](fields: Fields)(implicit conv: TupleConverter[(RowT, ValT)],
                                              setter: TupleSetter[(RowT, ValT)]) = {
    val vecPipe =
      RichPipe(pipe).mapTo(fields -> ('row, 'val))((tup: (RowT, ValT)) => tup)(conv, setter)
  ...
  def toRowVector[ColT, ValT](fields: Fields)(implicit conv: TupleConverter[(ColT, ValT)],
                                              setter: TupleSetter[(ColT, ValT)]) = {
    val vecPipe =
      RichPipe(pipe).mapTo(fields -> ('col, 'val))((tup: (ColT, ValT)) => tup)(conv, setter)
...
class MatrixMappableExtensions[T](mappable: Mappable[T])(implicit fd: FlowDef, mode: Mode) {
  ...
  def toRow[Row, Val](implicit ev: <:<[T, (Row, Val)],
                      setter: TupleSetter[(Row, Val)]): RowVector[Row, Val] = mapToRow {
    _.asInstanceOf[(Row, Val)]
  }

  def mapToRow[Row, Val](fn: (T) => (Row, Val))(implicit setter: TupleSetter[(Row, Val)],
  ...
  def toCol[Col, Val](implicit ev: <:<[T, (Col, Val)],
...
object Matrix {
  ...
  implicit def diagonalToMatrix[RowT, ValT](
...
class Matrix[RowT, ColT, ValT](val rowSym: Symbol,
                               val colSym: Symbol,
                               val valSym: Symbol,
                               inPipe: Pipe,
  ...
  def binarizeAs[NewValT](implicit mon: Monoid[ValT],
  ...
  def mapRows(fn: Iterable[(ColT, ValT)] => Iterable[(ColT, ValT)])(
    ...
    val newPipe = filterOutZeros(valSym, mon) {
      pipe
        ...
        .flatMapTo((rowSym, newListSym) -> (rowSym, colSym, valSym)) {
          tup: (RowT, List[(ColT, ValT)]) =>
            val row = tup._1
            val list = fn(tup._2)
            // Now flatten out to (row, col, val):
            list.map { imp: (ColT, ValT) =>
              (row, imp._1, imp._2)
            }
        }
    }
  ...
  def nonZerosWith[ValU](that: Scalar[ValU]): Matrix[RowT, ColT, (ValT, ValU)] = {
    ...
    val newPipe = inPipe
      ...
      .map(valSym.append(getField(newRFields, 0)) -> valSym) { leftRight: (ValT, ValU) =>
        Tuple1(leftRight)
      }
      .project(rowColValSymbols)
  ...
  def keepRowsBy[ValU](that: ColVector[RowT, ValU]): Matrix[RowT, ColT, ValT] = {
    ...
    val joined =
      pipe.joinWithSmaller(rowSym -> index, that.pipe.rename(that.rowS -> index).project(index))
    new Matrix[RowT, ColT, ValT](rowSym, colSym, valSym, joined.project(rowSym, colSym, valSym))
  }
  ...
  def keepColsBy[ValU](that: RowVector[ColT, ValU]): Matrix[RowT, ColT, ValT] = {
    val index = '____index____
    val joined =
      pipe.joinWithSmaller(colSym -> index, that.pipe.rename(that.colS -> index).project(index))
    new Matrix[RowT, ColT, ValT](rowSym, colSym, valSym, joined.project(rowSym, colSym, valSym))
  }
  ...
  def removeRowsBy[ValU](that: ColVector[RowT, ValU]): Matrix[RowT, ColT, ValT] = {
    val index = '____index____
    val joined = pipe.joinWithSmaller(rowSym -> index,
  ...
  def removeColsBy[ValU](that: RowVector[ColT, ValU]): Matrix[RowT, ColT, ValT] = {
    val index = '____index____
    val joined = pipe.joinWithSmaller(colSym -> index,
...
class DiagonalMatrix[IdxT, ValT](val idxSym: Symbol,
  ...
  def *[That, Res](that: That)(
      implicit prod: MatrixProduct[DiagonalMatrix[IdxT, ValT], That, Res]): Res = prod(this, that)
...
class RowVector[ColT, ValT](val colS: Symbol,
                            val valS: Symbol,
                            inPipe: Pipe,
  ...
  def *[That, Res](that: That)(
      implicit prod: MatrixProduct[RowVector[ColT, ValT], That, Res]): Res = prod(this, that)
...
class ColVector[RowT, ValT](val rowS: Symbol,
                            val valS: Symbol,
                            inPipe: Pipe,
                            val sizeH: SizeHint = FiniteHint(-1L, 1L))
    extends java.io.Serializable
  ...
  def *[That, Res](that: That)(
      implicit prod: MatrixProduct[ColVector[RowT, ValT], That, Res]): Res = prod(this, that)
  ...
  def mapWithIndex[ValNew](fn: (ValT, RowT) => ValNew)(
      implicit mon: Monoid[ValNew]): ColVector[RowT, ValNew] = transpose.mapWithIndex(fn).transpose
...
class BlockMatrix[RowT, GroupT, ColT, ValT](private val mat: Matrix[RowT, GroupT, Map[ColT, ValT]]) {
  def dotProd[RowT2](that: BlockMatrix[GroupT, RowT2, ColT, ValT])(
      implicit prod: MatrixProduct[Matrix[RowT, GroupT, Map[ColT, ValT]],
