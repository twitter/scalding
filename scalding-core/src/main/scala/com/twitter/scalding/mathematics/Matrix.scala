 ...
 * is generic, with the constraint that ValT is a Ring[T] In practice, RowT and ColT are going to be Strings,
 * Integers or Longs in the usual case.
 *
 * WARNING: It is NOT OKAY to use the same instance of Matrix/Row/Col with DIFFERENT Monoids/Rings/Fields. If
 * you want to change, midstream, the Monoid on your ValT, you have to construct a new Matrix. This is due to
 * caching of internal computation graphs.
 *
 * RowVector - handles matrices of row dimension one. It is the result of some of the matrix methods and has
 * methods that return ColVector and diagonal matrix
 *
 * ColVector - handles matrices of col dimension one. It is the result of some of the matrix methods and has
 * methods that return RowVector and diagonal matrix
 */
...
class MatrixPipeExtensions(pipe: Pipe) {
  def toMatrix[RowT, ColT, ValT](
    ...
    val matPipe =
      RichPipe(pipe).mapTo(fields -> ('row, 'col, 'val))((tup: (RowT, ColT, ValT)) => tup)(conv, setter)
  ...
  private def groupPipeIntoMap[ColT, ValT](pipe: Pipe): Pipe =
    pipe
      .groupBy('group, 'row) {
        ...
        }((l: Map[ColT, ValT], r: Map[ColT, ValT]) => l ++ r)((red: Map[ColT, ValT]) => red)
      }
      .rename('group, 'col)
  ...
  def mapToColVector[T, RowT, ValT](
      fields: Fields
  )(mapfn: T => (RowT, ValT))(implicit conv: TupleConverter[T], setter: TupleSetter[(RowT, ValT)]) = {
  ...
  def mapToRowVector[T, ColT, ValT](
      fields: Fields
  )(mapfn: T => (ColT, ValT))(implicit conv: TupleConverter[T], setter: TupleSetter[(ColT, ValT)]) = {
...
class MatrixMappableExtensions[T](mappable: Mappable[T])(implicit fd: FlowDef, mode: Mode) {
  def toMatrix[Row, Col, Val](implicit
      ev: <:<[T, (Row, Col, Val)],
      setter: TupleSetter[(Row, Col, Val)]
  ): Matrix[Row, Col, Val] =
    mapToMatrix(_.asInstanceOf[(Row, Col, Val)])

  def mapToMatrix[Row, Col, Val](
      fn: (T) => (Row, Col, Val)
  ...
  def mapToBlockMatrix[Group, Row, Col, Val](
      fn: (T) => (Group, Row, Col, Val)
  )(implicit ord: Ordering[(Group, Row)]): BlockMatrix[Group, Row, Col, Val] = {
  ...
  def toRow[Row, Val](implicit ev: <:<[T, (Row, Val)], setter: TupleSetter[(Row, Val)]): RowVector[Row, Val] =
    mapToRow(_.asInstanceOf[(Row, Val)])

  def mapToRow[Row, Val](
      fn: (T) => (Row, Val)
...
object Matrix {
  ...
  implicit def mappableExtensions[T](
      mt: Mappable[T]
  )(implicit fd: FlowDef, mode: Mode): MatrixMappableExtensions[T] =
...
class Matrix[RowT, ColT, ValT](
  ...
  /**
   * like zipWithIndex.map but ONLY CHANGES THE VALUE not the index. Note you will only see non-zero elements
   * on the matrix. This does not enumerate the zeros
   */
  def mapWithIndex[ValNew](
      fn: (ValT, RowT, ColT) => ValNew
  ...
  def mapRows(
      fn: Iterable[(ColT, ValT)] => Iterable[(ColT, ValT)]
  ...
  protected def topRowWithTiny(k: Int)(implicit ord: Ordering[ValT]): Matrix[RowT, ColT, ValT] = {
    ...
    val newPipe = pipe
      .groupBy(rowSym) {
        _.sortWithTake((colSym, valSym) -> 'top_vals, k)((t0: (ColT, ValT), t1: (ColT, ValT)) =>
          ord.gt(t0._2, t1._2)
        )
      }
  ...
  protected lazy val rowL1Norm = {
    val matD = this.asInstanceOf[Matrix[RowT, ColT, Double]]
    (matD.mapValues(x => x.abs).sumColVectors.diag.inverse) * matD
  }
  ...
  def elemWiseOp(that: Matrix[RowT, ColT, ValT])(fn: (ValT, ValT) => ValT)(implicit
      mon: Monoid[ValT]
  ): Matrix[RowT, ColT, ValT] =
  ...
  /**
   * Considering the matrix as a graph, propagate the column: Does the calculation: \sum_{j where M(i,j) ==
   * true) c_j
   */
  ...
  protected lazy val mainDiagonal: DiagonalMatrix[RowT, ValT] = {
    val diagPipe = pipe
      .filter(rowSym, colSym) { input: (RowT, RowT) =>
        (input._1 == input._2)
      }
   ...
   */
  private def cleanUpZipJoin[ValU](otherVSym: Fields, pairMonoid: Monoid[(ValT, ValU)])(
      joinedPipe: Pipe
  ): Pipe =
    joinedPipe
  ...
  def nonZerosWith[ValU](that: LiteralScalar[ValU]): Matrix[RowT, ColT, (ValT, ValU)] = {
    val newPipe = inPipe
      .map(valSym -> valSym) { left: Tuple1[ValT] =>
  ...
  def zip[ValU](
      that: ColVector[RowT, ValU]
  )(implicit pairMonoid: Monoid[(ValT, ValU)]): Matrix[RowT, ColT, (ValT, ValU)] = {
  ...
  def removeElementsBy[ValU](that: Matrix[RowT, ColT, ValU]): Matrix[RowT, ColT, ValT] = {
    ...
    val joined = pipe.joinWithSmaller(
      ...
      that.pipe.rename((that.rowSym, that.colSym, that.valSym) -> (filterR, filterC, filterV)),
      new LeftJoin
    )
...
class RowVector[ColT, ValT](
  ...
  def hProd(that: RowVector[ColT, ValT])(implicit ring: Ring[ValT]): RowVector[ColT, ValT] =
  ...
  /**
   * like zipWithIndex.map but ONLY CHANGES THE VALUE not the index. Note you will only see non-zero elements
   * on the vector. This does not enumerate the zeros
   */
  def mapWithIndex[ValNew](
      fn: (ValT, ColT) => ValNew
  )(implicit mon: Monoid[ValNew]): RowVector[ColT, ValNew] = {
    val newPipe = pipe
      .mapTo((valS, colS) -> (valS, colS)) { tup: (ValT, ColT) => (fn(tup._1, tup._2), tup._2) }
  ...
  protected def topWithTiny(k: Int)(implicit ord: Ordering[ValT]): RowVector[ColT, ValT] = {
    ...
    val newPipe = pipe
      .groupAll {
        _.sortWithTake((colS, valS) -> 'top_vals, k)((t0: (ColT, ValT), t1: (ColT, ValT)) =>
          ord.gt(t0._2, t1._2)
        )
      }
...
class ColVector[RowT, ValT](
  ...
  def hProd(that: ColVector[RowT, ValT])(implicit ring: Ring[ValT]): ColVector[RowT, ValT] =
  ...
  protected def topWithTiny(k: Int)(implicit ord: Ordering[ValT]): ColVector[RowT, ValT] = {
    ...
    val newPipe = pipe
      .groupAll {
        _.sortWithTake((rowS, valS) -> 'top_vals, k)((t0: (RowT, ValT), t1: (RowT, ValT)) =>
          ord.gt(t0._2, t1._2)
        )
      }
...
/**
 * BlockMatrix is 3 dimensional matrix where the rows are grouped It is useful for when we want to multiply
 * groups of vectors only between themselves. For example, grouping users by countries and calculating
 * products only between users from the same country
 */
