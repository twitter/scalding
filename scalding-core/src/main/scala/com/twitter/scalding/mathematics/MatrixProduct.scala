...
/**
 * TODO: Muliplication is the expensive stuff. We need to optimize the methods below: This object holds the
 * implicits to handle matrix products between various types
 */
object MatrixProduct extends java.io.Serializable {
  ...
  def getJoiner(leftSize: SizeHint, rightSize: SizeHint): MatrixJoiner = {
    ...
    if (SizeHintOrdering.lteq(leftSize, rightSize)) {
      ...
      leftSize.total
        .map(t => if (t < maxTinyJoin) TinyToAny else new SmallToBig(numOfReducers(newHint)))
  ...
  implicit def literalRightProduct[Row, Col, ValT](implicit
      ring: Ring[ValT]
  ...
  implicit def litScalarRowRightProduct[Col, ValT](implicit
      ring: Ring[ValT]
  ...
  implicit def litScalarColRightProduct[Row, ValT](implicit
      ring: Ring[ValT]
  ...
  implicit def scalarDiagRightProduct[Row, ValT](implicit
      ring: Ring[ValT]
  ...
  implicit def litScalarDiagRightProduct[Col, ValT](implicit
      ring: Ring[ValT]
  ...
  def vectorInnerProduct[IdxT, ValT](implicit
      ring: Ring[ValT]
