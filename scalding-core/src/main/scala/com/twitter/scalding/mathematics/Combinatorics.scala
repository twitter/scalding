...
/**
 * Serve as a repo for self-contained combinatorial functions with no dependencies such as combinations, aka n
 * choose k, nCk permutations , aka nPk subset sum : numbers that add up to a finite sum weightedSum: For
 * weights (a,b,c, ...), want integers (x,y,z,...) to satisfy constraint |ax + by + cz + ... - result | <
 * error ...
 *
 * @author
 *   : Krishnan Raman, kraman@twitter.com
 */
...
object Combinatorics {
   ...
   *
   * Use-case: Say you have 100 hashtags sitting in an array You want a table with 5 hashtags per row, all
   * possible combinations If the hashtags are sitting in a string array, then combinations[String]( hashtags,
   * 5) will create the 100 chose 5 combinations.
   *
   * Algorithm: Use k pipes, cross pipes two at a time, filter out non-monotonic entries
   *
   * eg. 10C2 = 10 choose 2 Use 2 pipes. Pipe1 = (1,2,3,...10) Pipe2 = (2,3,4....10) Cross Pipe1 with Pipe2
   * for 10*9 = 90 tuples Filter out tuples that are non-monotonic For (t1,t2) we want t1<t2, otherwise
   * reject. This brings down 90 tuples to the desired 45 tuples = 10C2
   */
   ...
   * multiples of the weight w ie. (0,1w,2w,3w,4w,....) Iterate as below - Cross two pipes Create a temp
   * column that stores intermediate results Apply progressive filtering on the temp column Discard the temp
   * column Once all pipes are crossed, test for temp column within error bounds of result Discard duplicates
   * at end of process
   *
   * Usecase: We'd like to generate all integer tuples for typical usecases like
   *
   * 0. How many ways can you invest $1000 in facebook, microsoft, hp ? val cash = 1000.0 val error = 5.0 //
   * max error $5, so its ok if we cannot invest the last $5 or less val (FB, MSFT, HP) = (23.3,27.4,51.2) //
   * share prices val stocks = IndexedSeq( FB,MSFT,HP ) weightedSum( stocks, cash, error).write(
   * Tsv("invest.txt"))
   *
   *   1. find all (x,y,z) such that 2x+3y+5z = 23, with max error 1 weightedSum( IndexedSeq(2.0,3.0,5.0),
   *      23.0, 1.0)
   *
   * 2. find all (a,b,c,d) such that 2a+12b+12.5c+34.7d = 3490 with max error 3 weightedSum(
   * IndexedSeq(2.0,12.0,2.5,34.7),3490.0,3.0)
   *
   * This is at the heart of portfolio mgmt( Markowitz optimization), subset-sum, operations-research LP
   * problems.
   */

  def weightedSum(weights: IndexedSeq[Double], result: Double, error: Double)(implicit
      flowDef: FlowDef,
      mode: Mode
  ): Pipe = {
  ...
  /**
   * Does the exact same thing as weightedSum, but filters out tuples with a weight of 0 The returned pipe
   * contain only positive non-zero weights.
   */
  def positiveWeightedSum(weights: IndexedSeq[Double], result: Double, error: Double)(implicit
      flowDef: FlowDef,
      mode: Mode
  ): Pipe = {
