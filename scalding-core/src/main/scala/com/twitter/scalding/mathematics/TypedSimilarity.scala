...
object GraphOperations extends Serializable {
   ...
   */
  def joinAggregate[N, E, T](
      grouped: Grouped[N, Edge[N, E]]
  )(agfn: Iterable[Edge[N, E]] => T): TypedPipe[Edge[N, (E, T)]] =
    grouped
      .cogroup(grouped) { (to: N, left: Iterator[Edge[N, E]], right: Iterable[Edge[N, E]]) =>
  ...
  def withInDegree[N, E](
      g: TypedPipe[Edge[N, E]]
  )(implicit ord: Ordering[N]): TypedPipe[Edge[N, (E, InDegree)]] = joinAggregate(g.groupBy(_.to)) { it =>
    InDegree(it.size)
  }
...
trait TypedSimilarity[N, E, S] extends Serializable {
  ...
  /**
   * Given a TypedPipe of edges, and a predicate for a smaller group (smallpred) of nodes and a bigger group
   * (bigpred), compute the similarity between each item in the two sets The Edge.from nodes in the result
   * will all satisfy smallpred, and the Edge.to will all satisfy bigpred. It is more efficient if you keep
   * the smallpred set smaller.
   */
  def apply(g: TypedPipe[Edge[N, E]], smallpred: N => Boolean, bigpred: N => Boolean): TypedPipe[Edge[N, S]]
...
object TypedSimilarity extends Serializable {
  ...
  def discoCosineSimilarity[N: Ordering](
    ...
    maybeWithReducers(
      smallG
        .cogroup(bigG) { (n: N, leftit: Iterator[(N, Int)], rightit: Iterable[(N, Int)]) =>
          // Use a co-group to ensure this happens in the reducer:
          leftit.flatMap { case (node1, deg1) =>
  ...
  def dimsumCosineSimilarity[N: Ordering](
    ...
    maybeWithReducers(
      smallG
        .cogroup(bigG) {
          (n: N, leftit: Iterator[(N, Double, Double)], rightit: Iterable[(N, Double, Double)]) =>
            // Use a co-group to ensure this happens in the reducer:
            leftit.flatMap { case (node1, weight1, norm1) =>
...
/**
 * This algothm is just matrix multiplication done by hand to make it clearer when we do the sampling
 * implementation
 */
class ExactInCosine[N](reducers: Int = -1)(implicit override val nodeOrdering: Ordering[N])
...
/**
 * Params: minCos: the minimum cosine similarity you care about accuracy for delta: the error on the
 * approximated cosine (e.g. 0.05 = 5%) boundedProb: the probability we have larger than delta error see:
 * https://arxiv.org/pdf/1206.2082v2.pdf for more details
 */
class DiscoInCosine[N](minCos: Double, delta: Double, boundedProb: Double, reducers: Int = -1)(implicit
...
class DimsumInCosine[N](minCos: Double, delta: Double, boundedProb: Double, reducers: Int = -1)(implicit
    override val nodeOrdering: Ordering[N]
