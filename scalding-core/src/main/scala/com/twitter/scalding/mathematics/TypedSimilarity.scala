      ...
      g.join(g)
        ...
        .flatMap {
          case ((node1, deg1), (node2, deg2)) =>
            if (smallpred(node1) && bigpred(node2)) Some(((node1, node2), (1, deg1, deg2)))
            else None
        }
        .group,
      g.reducers
 ...
 */
class DiscoInCosine[N](minCos: Double, delta: Double, boundedProb: Double, reducers: Int = -1)(
...
class DimsumInCosine[N](minCos: Double, delta: Double, boundedProb: Double, reducers: Int = -1)(
    implicit override val nodeOrdering: Ordering[N])
