...
object FileSource {
   ...
   */
  def allGlobFilesWithSuccess(globPath: String,
                              conf: Configuration,
    ...
    mode match {
      ...
      case Local(strict) => {
        ...
        if (strict && !files.forall(_.exists)) {
          throw new InvalidSourceException(
            "[" + this.toString + s"] Data is missing from: ${localPaths.filterNot { p =>
...
class ScaldingMultiSourceTap(taps: Seq[Tap[JobConf, RecordReader[_, _], OutputCollector[_, _]]])
    extends MultiSourceTap[Tap[JobConf, RecordReader[_, _], OutputCollector[_, _]],
                           JobConf,
                           RecordReader[_, _]](taps: _*) {
...
trait TextSourceScheme extends SchemedSource {
  ...
  override def hdfsScheme =
    HadoopSchemeInstance(new CHTextLine(CHTextLine.DEFAULT_SOURCE_FIELDS, textEncoding))
}
 ...
 * cascading docs.
 */
case class MultipleTsvFiles(p: Seq[String],
                            override val fields: Fields = Fields.ALL,
                            override val skipHeader: Boolean = false,
                            override val writeHeader: Boolean = false)
...
object TextLine {
  ...
  def apply(p: String,
            ...
            textEncoding: String = defaultTextEncoding): TextLine =
...
object OffsetTextLine {
  ...
  def apply(p: String,
            sm: SinkMode = defaultSinkMode,
            textEncoding: String = defaultTextEncoding): OffsetTextLine =
