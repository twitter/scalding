...
/**
 * Allows working with an iterable object defined in the job (on the submitter) to be used within a Job as you
 * would a Pipe/RichPipe
 *
 * These lists should probably be very tiny by Hadoop standards. If they are getting large, you should
 * probably dump them to HDFS and use the normal mechanisms to address the data (a FileSource).
 */
case class IterableSource[+T](@transient iter: Iterable[T], inFields: Fields = Fields.NONE)(implicit
    set: TupleSetter[T],
  ...
  /**
   * Don't use the whole string of the iterable, which can be huge. We take the first 10 items + the
   * identityHashCode of the iter.
   */
