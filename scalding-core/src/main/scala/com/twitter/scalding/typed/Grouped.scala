...
object Grouped {
  ...
  def addEmptyGuard[K, V1, V2](
      fn: (K, Iterator[V1]) => Iterator[V2]): (K, Iterator[V1]) => Iterator[V2] = {
          ...
          val (sortOpt, ts) = valueSort
            .map {
              case ordser: OrderedSerialization[V1] =>
                ...
                val ts2 = tupleSetter.asInstanceOf[TupleSetter[(K, Boxed[V1])]].contraMap {
                  kv1: (K, V1) =>
                    (kv1._1, boxfn(kv1._2))
                }
                (Some(valueF), ts2)
...
case class UnsortedIdentityReduce[K, V1](override val keyOrdering: Ordering[K],
                                         override val mapped: TypedPipe[(K, V1)],
                                         override val reducers: Option[Int],
                                         override val descriptions: Seq[String])
    extends ReduceStep[K, V1]
  ...
  override def bufferedTake(n: Int) =
    if (n < 1) {
      ...
      UnsortedIdentityReduce[K, V1](keyOrdering, pretake, reducers, descriptions).forceToReducers // jump to ValueSortedReduce
        .take(n)
    }
...
case class IdentityValueSortedReduce[K, V1](override val keyOrdering: Ordering[K],
                                            override val mapped: TypedPipe[(K, V1)],
  ...
  override def reverse: IdentityValueSortedReduce[K, V1] =
    IdentityValueSortedReduce[K, V1](keyOrdering,
                                     mapped,
                                     valueSort.reverse,
                                     reducers,
                                     descriptions)
  ...
  override def bufferedTake(n: Int): SortedGrouped[K, V1] =
    if (n <= 0) {
      ...
      val pretake: TypedPipe[(K, V1)] = mapped
        ...
        .flatMap { case (k, vs) => vs.iterator.asScala.map((k, _)) }
      // Now finish on the reducers
      IdentityValueSortedReduce[K, V1](keyOrdering, pretake, valueSort, reducers, descriptions).forceToReducers // jump to ValueSortedReduce
        .take(n)
    }
...
case class ValueSortedReduce[K, V1, V2](override val keyOrdering: Ordering[K],
                                        override val mapped: TypedPipe[(K, V1)],
                                        valueSort: Ordering[_ >: V1],
  ...
  override def withDescription(description: String) =
    ValueSortedReduce[K, V1, V2](keyOrdering,
                                 mapped,
                                 valueSort,
                                 reduceFn,
                                 reducers,
...
case class IteratorMappedReduce[K, V1, V2](override val keyOrdering: Ordering[K],
                                           override val mapped: TypedPipe[(K, V1)],
                                           reduceFn: (K, Iterator[V1]) => Iterator[V2],
                                           override val reducers: Option[Int],
                                           override val descriptions: Seq[String])
    ...
    groupOp {
      _.every(
        new cascading.pipe.Every(
          _,
          Grouped.valueField,
          new TypedBufferOp(Grouped.keyConverter(keyOrdering),
