...
object ValuePipe {
  ...
  def fold[T, U, V](l: ValuePipe[T], r: ValuePipe[U])(f: (T, U) => V) =
    new ComputedValue(l.toPipe.cross(r.toPipe).map{ case (lv, rv) => f(lv, rv) })

  implicit def semigroup[T](implicit sg: Semigroup[T]) = new Semigroup[ValuePipe[T]] {
...
case class LiteralValue[T](value: T)(implicit val flowDef: FlowDef, mode: Mode) extends ValuePipe[T] {
  ...
  lazy val toPipe = TypedPipe.from(IterableSource(Iterable(value)))
}
