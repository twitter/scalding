 ...
 *
 * For writing it expects a pair of `(P, String)`, where `P` is the data used for partitioning and `String` is
 * the output to write out. Below is an example.
 * {{{
 * val data = List(
 *   (("a", "x"), "line1"),
 *   (("a", "y"), "line2"),
 ...
 * }}}
 *
 * For reading it produces a pair `(P, (Long, String))` where `P` is the partition data, `Long` is the offset
 * into the file and `String` is a line from the file. Below is an example.
 * {{{
