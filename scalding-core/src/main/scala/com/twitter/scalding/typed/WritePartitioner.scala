...
object WritePartitioner {
   ...
   *
   * This is really only useful for jobs with single final outputs since you want to partition the entire job,
   * not a portion of it.
   */
  ...
  /**
   * This enables us to write the partitioning in terms of this applicative type that is equipped with two
   * extra operations: materialized and write, but not a general flatMap
   *
   * so the only sequencing power we have is to materialize
   *
   * This allows us to test the properties we want without having to deal with Execution, which is a black box
  ...
  def materialize1[M[+_], S[_]](phases: Seq[Rule[TypedPipe]], ws: List[PairK[TypedPipe, S, _]])(implicit
      mat: Materializer[M]
  ): List[PairK[mat.TP, S, _]] = {
    val e = Dag.empty(OptimizationRules.toLiteral)

    logger.info(s"converting ${ws.size} writes into several parts")
    val (finalDag, writeIds) = ws.foldLeft((e, List.empty[PairK[Id, S, _]])) { case ((dag, writes), pair) =>
    ...
    def handleHashCoGroup[K, V, V2, R](
      ...
      val exright: M[HashJoinable[K, V2]] = hj.right match {
        ...
        case step @ UnsortedIdentityReduce(_, _, _, _, _) =>
          ...
          val mappedV2 = step.evidence.subst[TK](step.mapped)
          mat.map(recurse(mappedV2)) { (tp: TypedPipe[(K, V2)]) =>
            UnsortedIdentityReduce[K, V2, V2](
              step.keyOrdering,
              tp,
              step.reducers,
              step.descriptions,
    ...
    def handleReduceStep[K, V1, V2](
        rs: ReduceStep[K, V1, V2],
        recurse: FunctionK[TypedPipe, mat.TP]
    ): mat.TP[(K, V2)] =
      mat.map(recurse(rs.mapped)) { pipe =>
        TypedPipe.ReduceStepPipe(ReduceStep.setInput[K, V1, V2](rs, pipe))
      }
    ...
    /**
     * If cascading would consider the current pipe as a Logical reduce we can avoid some forces below. This
     * method returns true if the pipe is ending on a reduce (not potentially a map-only job)
     */
    @annotation.tailrec
    ...
    /**
     * We use this state to track where we are as we recurse up the graph. Since we know at the very end we
     * will write, we can avoid, for instance forcing a reduce operation that is followed only by a map and a
     * write.
     *
     * Coupled with the isLogicalReduce above, we can emulate the behavior of the cascading planner as we
     * recurse up.
     */
    ...
    /**
     * Given a pipe, and the state below it, return the materialized version of that pipe. This should cause
     * no more materializations than cascading would do, and indeed we test for this property
     */
    val fn = Memoize.functionK[P, mat.TP](new Memoize.RecursiveK[P, mat.TP] {
      import TypedPipe._
      import BelowState._
