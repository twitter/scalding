...
object TDsl extends Serializable with GeneratedTupleAdders {
  implicit def pipeTExtensions(pipe: Pipe)(implicit flowDef: FlowDef): PipeTExtensions =
    new PipeTExtensions(pipe, flowDef)

  implicit def mappableToTypedPipe[T](src: Mappable[T])(implicit mode: Mode): TypedPipe[T] =
    TypedPipe.from(src)(mode)

  implicit def sourceToTypedPipe[T](src: TypedSource[T])(implicit mode: Mode): TypedPipe[T] =
    TypedPipe.from(src)(mode)
}
 ...
 */
class PipeTExtensions(pipe: Pipe, flowDef: FlowDef) extends Serializable {
  ...
  def typed[T, U](fielddef: (Fields, Fields))(fn: TypedPipe[T] => TypedPipe[U])(implicit conv: TupleConverter[T], setter: TupleSetter[U]): Pipe =
    fn(TypedPipe.from(pipe, fielddef._1)(flowDef, conv)).toPipe(fielddef._2)(flowDef, setter)
