...
object NoStackAndThen {
  ...
  private case class NoStackMore[A, B, C](first: NoStackAndThen[A, B], andThenFn: (B) => C) extends NoStackAndThen[A, C] {
     ...
     */
    @annotation.tailrec
    private def reversed(toPush: NoStackAndThen[A, Any], rest: ReversedStack[Any, C]): ReversedStack[Any, C] =
      toPush match {
        case NoStackWrap(fn) => NonEmpty(fn, rest)
        case NoStackMore(more, fn) => reversed(more.asInstanceOf[NoStackAndThen[A, Any]], NonEmpty(fn, rest))
      }
    @annotation.tailrec
    ...
    private lazy val revStack =
      // casts needed for the tailrec
      reversed(first.asInstanceOf[NoStackAndThen[A, Any]], EmptyStack(andThenFn.asInstanceOf[(Any) => (C)]))
