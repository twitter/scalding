 ...
 * output to write out. Below is an example.
 * {{{
 * val data = List(
 *   (("a", "x"), ("i", 1)),
 *   (("a", "y"), ("j", 2)),
 ...
 * }}}
 *
 * For reading it produces a pair `(P, T)` where `P` is the partition data and `T` is data in the files. Below
 * is an example.
 * {{{
  ...
  override def hdfsScheme = {
    val scheme =
      HadoopSchemeInstance(
        new TextDelimited(fields, null, skipHeader, writeHeader, separator, strict, quote, types, safe)
...
/**
 * Trait to assist with creating objects such as [[PartitionedTsv]] to read from separated files. Override
 * separator, skipHeader, writeHeader as needed.
 */
trait PartitionedDelimited extends Serializable {
  ...
  def apply[P: Manifest: TupleConverter: TupleSetter, T: Manifest: TupleConverter: TupleSetter](
      path: String,
      template: String
  ): PartitionedDelimitedSource[P, T] =
