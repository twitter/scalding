...
object CascadingBackend {
  ...
  /**
   * If we are using OrderedComparable, we need to box the key to prevent other serializers from handling the
   * key
   */
  private def getBoxFnAndOrder[K](
      ordser: OrderedSerialization[K],
      flowDef: FlowDef
  ): (K => Boxed[K], BoxedOrderedSerialization[K]) = {
    // We can only supply a cacheKey if the equals and hashcode are known sane
    val (boxfn, cls) =
      Boxed.nextCached[K](if (ordser.isInstanceOf[EquivSerialization[_]]) Some(ordser) else None)
  ...
  /**
   * Check if the Ordering is an OrderedSerialization, if so box in a Boxed so hadoop and cascading can
   * dispatch the right serialization
   */
  private def maybeBox[K, V](ord: Ordering[K], flowDef: FlowDef)(
      op: (TupleSetter[(K, V)], Fields) => Pipe
  ): Pipe =
    ord match {
  ...
  private case class CascadingPipe[+T](
    ...
    /**
     * merge the flowDef into this new flowdef an make sure the tuples have the structure defined by setter
     */
  ...
  /**
   * we want to cache renderings of some TypedPipe to Pipe so cascading will see them as the same. Without
   * this, it is very easy to have a lot of recomputation. Ideally we would plan an entire graph at once, and
   * not need a static cache here, but currently we still plan one TypedPipe at a time.
   */
   ...
   *
   * Since equal `TypedPipe`s define same computation we would like to compile them into referentially the
   * same cascading's `Pipe` instance. This logic breaks if one typed pipe is really big and has two forked
   * different computations both of which significantly decrease size of the data. If we will cache common
   * part of this two computations in the same cascading's `Pipe` instance we end up with common part being
   * materialized. Therefore for some kind of `TypedPipe`s we want to avoid their caching.
   *
   * `.cross` `TypedPipe` is one of examples of such `TypedPipe`s we never want to materialize and, therefore,
   * cache.
   *
   * `compile` logic is separated into next functions:
   *   - `transform` which defines main transformation logic, without any caching applied. This method accepts
   *     `rec` parameter which is being called to transform children pipes.
   *   - `withCachePolicy` which defines transformation logic with caching applied.
   *   - `notCached` to support use case with `.cross` pipe, where pipe itself shouldn't be cached but `left`
   *     and `right` sides of it should be.
   */
  ...
  final def toPipe[U](p: TypedPipe[U], fieldNames: Fields)(implicit
      flowDef: FlowDef,
      mode: Mode,
  ...
  /**
   * This needs to be called, to plan any writes from the TypedPipe API. This is here to globally optimize the
   * entire flow, and not optimize on a write-by-write basis.
   *
   * This uses the FlowStateMap this method is idempotent.
   *
   * It is called by default in ExecutionContext at the last step before building the Flow. Job also needs to
   * call this method in validate to make sure validation works.
   */
  def planTypedWrites(fd: FlowDef, mode: Mode): Unit = {
    ...
    if (pendingWritesAfterPlan.nonEmpty) {
      logger.warn(
        "Using Typed API in TypedSink implementation is prohibited and " +
   ...
   *
   * Return None if the Execution is empty. If the FlowDef includes things other than TypedPipe.writes, this
   * will return Some failed Execution.
   *
   * This method is useful for people who have used the Typed-API with FlowDefs, but not Executions and want
   * to convert to an Execution without rewriting all the code. An example of this is summingbird which
   * constructs a single FlowDef for the entire plan it makes. For large plans from summingbird you may want
   * to use write partitioning.
   */
  def flowDefToExecution(
      fd: FlowDef,
      partitionOptimizations: Option[Seq[Rule[TypedPipe]]]
  ): Option[Execution[Unit]] = {
    ...
    if (rfd.isEmpty) {
      FlowStateMap
        .get(fd)
        .flatMap { // Note, we want this to be a pure function so we don't mutate the FlowStateMap
            ...
            writes match {
              ...
              case nonEmpty =>
                partitionOptimizations match {
                  ...
                  case Some(rules) =>
                    def toPair[A](
                        f: FlowStateMap.TypedWrite[A]
                    ): WritePartitioner.PairK[TypedPipe, TypedSink, A] =
                      (f.pipe, f.sink)
      ...
      Some(
        Execution.failed(
          new Exception(s"We can only convert Typed-API Jobs to Execution. Found non-empty FlowDef: $fd")
        )
      )
  }

  /**
   * This converts the TypedPipe to a cascading Pipe doing the most direct possible translation we can. This
   * is useful for testing or for expert cases where you want more direct control of the TypedPipe than the
   * default method gives you.
   */
  final def toPipeUnoptimized[U](input: TypedPipe[U], fieldNames: Fields)(implicit
      flowDef: FlowDef,
      mode: Mode,
      setter: TupleSetter[U]
  ): Pipe = {
    ...
    /**
     * These rules are not optimizations, but actually required for Cascading to not throw. Cascading requires
     * certain shapes of the graphs
     */
  ...
  private def planCoGroup[K, R](
    ...
    val newPipe = maybeBox[K, Any](ord, flowDef) { (tupset, ordKeyField) =>
      if (firstCount == inputs.size) {
        ...
        /**
         * This is a self-join Cascading handles this by sending the data only once, spilling to disk if the
         * groups don't fit in RAM, then doing the join on this one set of data. This is fundamentally
         * different than the case where the first item is not repeated. That case is below
         */
        ...
        /**
         * As long as the first one appears only once, we can handle self joins on the others: Cascading does
         * this by maybe spilling all the streams other than the first item. This is handled by a different
         * CoGroup constructor than the above case.
         */
        ...
        val pipes: Array[Pipe] = distincts.zipWithIndex.map { case (item, idx) =>
        ...
        /**
         * This is non-trivial to encode in the type system, so we throw this exception at the planning phase.
         */
        sys.error(
    ...
    CascadingPipe[(K, R)](pipeWithRedAndDescriptions, kvFields, flowDef, tuple2Converter[K, R])
  }

  /**
   * TODO: most of the complexity of this method should be rewritten as an optimization rule that works on the
   * scalding typed AST. the code in here gets pretty complex and depending on the details of cascading and
   * also how we compile to cascading.
   *
   * But the optimization is somewhat general: we often want a checkpoint before a hashjoin is replicated
   */
  private def planHashJoin[K, V1, V2, R](
    ...
    CascadingPipe[(K, R)](RichPipe(hashPipe).project(kvFields), kvFields, fd, tuple2Converter[K, R])
  }
