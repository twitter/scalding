...
object RatioBasedEstimator {
  ...
  /**
   * RatioBasedEstimator optionally ignores history items whose input size is drastically different than the
   * current job. This parameter specifies the lower bound on allowable input size ratio. Defaults to 0.10
   * (10%), which sets the upper bound to 10x.
   */
...
abstract class RatioBasedEstimator extends ReducerHistoryEstimator {
  ...
  /**
   * Determines if this input and the previous input are close enough. If they're drastically different, we
   * have no business trying to make an estimate based on the past job.
   *
   * @param threshold
   *   Specify lower bound on ratio (e.g. 0.10 for 10%)
   */
  ...
  /**
   * Compute the average ratio of mapper bytes to reducer bytes and use that to scale the estimate produced by
