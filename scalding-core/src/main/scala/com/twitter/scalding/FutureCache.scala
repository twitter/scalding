...
class FutureCache[-K, V, P[_], F[_]](implicit pl: PromiseLike[P, F]) {
  ...
  def getOrElseUpdate(k: K, res: => F[V])(implicit cec: ConcurrentExecutionContext): F[V] =
    getOrElseUpdateIsNew(k, res)._2

  /**
   * Tells you if this was the first lookup of this key or not
   */
  def getOrElseUpdateIsNew(k: K, res: => F[V])(implicit cec: ConcurrentExecutionContext): (Boolean, F[V]) =
   ...
   */
  def getOrPromise(k: K)(implicit cec: ConcurrentExecutionContext): Either[P[V], F[V]] = {
    /*
