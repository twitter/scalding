...
abstract class DailyPrefixSuffixSource(prefixTemplate: String,
    ...
    extends TimePathedSource(
      prefixTemplate + TimePathedSource.YEAR_MONTH_DAY + suffixTemplate + "/*",
      dateRange,
      DateOps.UTC)
...
abstract class DailySuffixMostRecentSource(prefixTemplate: String, dateRange: DateRange)
    extends MostRecentGoodSource(prefixTemplate + TimePathedSource.YEAR_MONTH_DAY + "/*",
                                 dateRange,
                                 DateOps.UTC)

object DailySuffixTsv {
  def apply(prefix: String, fs: Fields = Fields.ALL)(implicit dateRange: DateRange) =
...
class DailySuffixTypedTsv[T](prefix: String)(implicit override val dateRange: DateRange,
