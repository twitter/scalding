...
object MultiJoinExternalizer {
  ...
  final case class ExternalizeMapGroup[A, B, C](@transient fn: (A, Iterator[B]) => Iterator[C]) extends Function2[A, Iterator[B], Iterator[C]] {
    private[this] val fnEx = Externalizer(fn)

    def apply(a: A, it: Iterator[B]) = fnEx.get(a, it)
  }

  final case class ExternalizeJoin[A, B, C, D](@transient fn: (A, Iterator[B], Iterable[C]) => Iterator[D]) extends Function3[A, Iterator[B], Iterable[C], Iterator[D]] {
    private[this] val fnEx = Externalizer(fn)
