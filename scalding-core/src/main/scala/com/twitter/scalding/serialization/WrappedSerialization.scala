...
import org.apache.hadoop.io.serializer.{Deserializer, Serialization => HSerialization, Serializer}
import org.apache.hadoop.conf.{Configurable, Configuration}

import java.io.{InputStream, OutputStream}
import com.twitter.bijection.{Base64String, Injection, JavaSerializationInjection}
import scala.collection.JavaConverters._

/**
 * WrappedSerialization wraps a value in a wrapper class that has an associated Binary that is used to
 * deserialize items wrapped in the wrapper
 */
...
object WrappedSerialization {
  ...
  private def getSerializer[U]: Injection[Externalizer[U], String] = {
    implicit val initialInj: Injection[Externalizer[U], Array[Byte]] =
      JavaSerializationInjection[Externalizer[U]]
