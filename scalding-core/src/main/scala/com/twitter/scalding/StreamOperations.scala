...
/**
 * Implements reductions on top of a simple abstraction for the Fields-API We use the f-bounded polymorphism
 * trick to return the type called Self in each operation.
 */
trait StreamOperations[+Self <: StreamOperations[Self]] extends Sortable[Self] with java.io.Serializable {
   ...
   * issue.
   *
   * WARNING: Any fields not referenced by the input fields will be aligned to the first output, and the final
   * hadoop stream will have a length of the maximum of the output of this, and the input stream. So, if you
   * change the length of your inputs, the other fields won't be aligned. YOU NEED TO INCLUDE ALL THE FIELDS
   * YOU WANT TO KEEP ALIGNED IN THIS MAPPING! POB: This appears to be a Cascading design decision.
   *
  ...
  )(init: X)(fn: (X, T) => X)(implicit setter: TupleSetter[X], conv: TupleConverter[T]): Self =
