...
trait LowPriorityFieldConversions {
  ...
  /**
   * Handles treating any TupleN as a Fields object. This is low priority because List is also a Product, but
   * this method will not work for List (because List is Product2(head, tail) and so productIterator won't
   * work as expected. Lists are handled by an implicit in FieldConversions, which have higher priority.
   */
...
trait FieldConversions extends LowPriorityFieldConversions {
  ...
  /**
   * Rather than give the full power of cascading's selectors, we have a simpler set of rules encoded below:
   * 1) if the input is non-definite (ALL, GROUP, ARGS, etc...) ALL is the output. Perhaps only fromFields=ALL
   * will make sense 2) If one of from or to is a strict super set of the other, SWAP is used. 3) If they are
   * equal, REPLACE is used. 4) Otherwise, ALL is used.
   */
  ...
  /**
   * Useful to convert f : Any* to Fields. This handles mixed cases ("hey", 'you). Not sure we should be this
   * flexible, but given that Cascading will throw an exception before scheduling the job, I guess this is
   * okay.
   */
  ...
  /**
   * We can't set the field Manifests because cascading doesn't (yet) expose field type information in the
   * Fields API.
   */
  implicit def fieldsToRichFields(fields: Fields): RichFields = {
    ...
    new RichFields(
      asList(fields).zip(fields.getComparators).map { case (id: Comparable[_], comparator: Comparator[_]) =>
        id match {
...
final case class IntField[T](override val id: java.lang.Integer)(implicit
...
final case class StringField[T](override val id: String)(implicit
    override val ord: Ordering[T],
    override val mf: Option[Manifest[T]]
) extends Field[T]
...
object Field {
  ...
  def apply[T](symbol: Symbol)(implicit ord: Ordering[T], mf: Manifest[T]) =
    StringField[T](symbol.name)(ord, Some(mf))
