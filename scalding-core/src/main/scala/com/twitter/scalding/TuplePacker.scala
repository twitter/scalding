...
/**
 * Typeclass for packing a cascading Tuple into some type T, this is used to put fields of a cascading tuple
 * into Thrift, Protobuf, or case classes, for instance, but you can add your own instances to control how
 * this is done.
 *
 * @author
 *   Argyris Zymnis
...
trait CaseClassPackers extends LowPriorityTuplePackers {
  implicit def caseClassPacker[T <: Product](implicit mf: Manifest[T]): OrderedTuplePacker[T] =
    new OrderedTuplePacker[T]
}
...
/**
 * Packs a tuple into any object with set methods, e.g. thrift or proto objects. TODO: verify that protobuf
 * setters for field camel_name are of the form setCamelName. In that case this code works for proto.
 *
 * @author
 *   Argyris Zymnis
 * @author
