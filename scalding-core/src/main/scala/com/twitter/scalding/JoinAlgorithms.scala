...
trait JoinAlgorithms {
  ...
  /**
   * This method is used internally to implement all joins. You can use this directly if you want to implement
   * something like a star join, e.g., when joining a single pipe to multiple other pipes. Make sure that you
   * call this method on the larger pipe to make the grouping as efficient as possible.
   *
   * If you are only joining two pipes, then you are better off using
   * joinWithSmaller/joinWithLarger/joinWithTiny/leftJoinWithTiny.
   */
  ...
  /**
   * ==WARNING==
   * Doing a cross product with even a moderate sized pipe can create ENORMOUS output. The use-case here is
   * attaching a constant (e.g. a number or a dictionary or set) to each row in another pipe. A common
   * use-case comes from a groupAll and reduction to one row, then you want to send the results back out to
   * every element in a pipe
   *
   * This uses joinWithTiny, so tiny pipe is replicated to all Mappers. If it is large, this will blow up. Get
   * it: be foolish here and LOSE IT ALL!
   *
  ...
  /**
   * Does a cross-product by doing a blockJoin. Useful when doing a large cross, if your cluster can take it.
   ...
   */
  private def renameCollidingFields(
      p: Pipe,
      fields: Fields,
      collisions: Set[Comparable[_]]
  ): (Pipe, Fields, Fields) = {
  ...
  /**
   * Joins the first set of keys in the first pipe to the second set of keys in the second pipe. All keys must
   * be unique UNLESS it is an inner join, then duplicated join keys are allowed, but the second copy is
   * deleted (as cascading does not allow duplicated field names).
   *
   * Smaller here means that the values/key is smaller than the left.
   *
   * Avoid going crazy adding more explicit join modes. Instead do for some other join mode with a larger
   * pipe:
   *
   * {{{
   * .then { pipe => other.
  ...
  /**
   * This is joinWithSmaller with joiner parameter fixed to LeftJoin. If the item is absent on the right put
   * null for the keys and values
   */
  def leftJoinWithSmaller(fs: (Fields, Fields), that: Pipe, reducers: Int = -1) =
    joinWithSmaller(fs, that, new LeftJoin, reducers)

  /**
   * This is joinWithLarger with joiner parameter fixed to LeftJoin. If the item is absent on the right put
   * null for the keys and values
   */
  ...
  /**
   * This does an assymmetric join, using cascading's "HashJoin". This only runs through this pipe once, and
   * keeps the right hand side pipe in memory (but is spillable).
   *
   * Choose this when Left > max(mappers,reducers) * Right, or when the left side is three orders of magnitude
   * larger.
   *
   * joins the first set of keys in the first pipe to the second set of keys in the second pipe. Duplicated
   * join keys are allowed, but the second copy is deleted (as cascading does not allow duplicated field
   * names).
   *
   * ==Warning==
  ...
  /**
   * Performs a block join, otherwise known as a replicate fragment join (RF join). The input params
   * leftReplication and rightReplication control the replication of the left and right pipes respectively.
   *
   * This is useful in cases where the data has extreme skew. A symptom of this is that we may see a job stuck
   * for a very long time on a small number of reducers.
   *
   * A block join is way to get around this: we add a random integer field and a replica field to every tuple
   * in the left and right pipes. We then join on the original keys and on these new dummy fields. These dummy
   * fields make it less likely that the skewed keys will be hashed to the same reducer.
   *
   * The final data size is right * rightReplication + left * leftReplication but because of the
   * fragmentation, we are guaranteed the same number of hits as the original join.
   *
   * If the right pipe is really small then you are probably better off with a joinWithTiny. If however the
   * right pipe is medium sized, then you are better off with a blockJoinWithSmaller, and a good rule of thumb
   * is to set rightReplication = left.size / right.size and leftReplication = 1
   *
   * Finally, if both pipes are of similar size, e.g. in case of a self join with a high data skew, then it
   * makes sense to set leftReplication and rightReplication to be approximately equal.
   *
   * ==Note==
   * You can only use an InnerJoin or a LeftJoin with a leftReplication of 1 (or a RightJoin with a
   * rightReplication of 1) when doing a blockJoin.
   */
   ...
   */
  private def addReplicationFields(
      p: Pipe,
      f: Fields,
      replication: Int,
    ...
    /**
     * We need to seed exactly once and capture that seed. If we let each task create a seed, a restart will
     * change the computation, and this could result in subtle bugs.
     */
   ...
   *
   * For example, suppose you have two pipes P1 and P2. While performing a skewed join for a particular key K,
   * you want to replicate every row in P1 with this key 3 times, and every row in P2 with this key 5 times.
   *
   * Then:
   *
   ...
   *
   * For example, imagine joining a pipe of Twitter's follow graph against a pipe of user genders, in order to
   * find the gender distribution of the accounts every Twitter user follows. Since celebrities (e.g., Justin
   * Bieber and Lady Gaga) have a much larger follower base than other users, and (under a standard join
   * algorithm) all their followers get sent to the same reducer, the job will likely be stuck on a few
   * reducers for a long time. A skewed join attempts to alleviate this problem.
   *
   * This works as follows:
   *
   *   1. First, we sample from the left and right pipes with some small probability, in order to determine
   *      approximately how often each join key appears in each pipe. 2. We use these estimated counts to
   *      replicate the join keys, according to the given replication strategy. 3. Finally, we join the
   *      replicated pipes together.
   *
   * @param sampleRate
   ...
   */
  def skewJoinWithSmaller(
      fs: (Fields, Fields),
      otherPipe: Pipe,
      sampleRate: Double = 0.001,
      reducers: Int = -1,
      replicator: SkewReplication = SkewReplicationA()
  ): Pipe = {
    ...
    val replicatedLeft = skewReplicate(
      pipe,
      sampledCounts,
      fs._1,
      sampledCountFields,
    ...
    val replicatedRight = skewReplicate(
      rightPipe,
      sampledCounts,
  ...
  /**
   * Helper method for performing skewed joins. This replicates the rows in {pipe} according to the estimated
   * counts in {sampledCounts}.
   *
   * @param pipe
   *   The pipe to be replicated.
   * @param sampledCounts
   *   A pipe containing, for each key, the estimated counts of how often this key appeared in the samples of
   *   the original left and right pipes.
   * @param replicator
   *   Strategy for how the pipe is replicated.
   * @param isPipeOnRight
   *   Set to true when replicating the right pipe.
   */
  private def skewReplicate(
      pipe: Pipe,
      sampledCounts: Pipe,
      joinFields: Fields,
      countFields: Fields,
...
/**
 * Wraps a Joiner instance so that the active FlowProcess may be noted. This allows features of Scalding that
 * need access to a FlowProcess (e.g., counters) to function properly inside a Joiner.
 */
