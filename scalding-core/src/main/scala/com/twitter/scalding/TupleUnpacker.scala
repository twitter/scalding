...
/**
 * Typeclass for objects which unpack an object into a tuple. The packer can verify the arity, types, and also
 * the existence of the getter methods at plan time, without having the job blow up in the middle of a run.
 *
 * @author
 *   Argyris Zymnis
 * @author
...
object ReflectionUtils {
  ...
  /**
   * Returns the set of fields in the given class. We use a List to ensure fields are in the same order they
   * were declared.
   */
  ...
  /**
   * For a given class, give a function of T, fieldName, fieldValue that returns a new T (possibly a copy, if
   * T is immutable).
   */
