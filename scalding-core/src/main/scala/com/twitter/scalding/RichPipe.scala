...
object RichPipe extends java.io.Serializable {
  ...
  private[scalding] val FromUuidPattern =
   ...
   * @param p
   *   a pipe, whose name should be changed
   * @return
   *   a pipe with a new name which is guaranteed to be new and never re-assigned by this function
   *
  ...
  /**
   * Gets the underlying config for this pipe and sets the number of reducers useful for cascading
   * GroupBy/CoGroup pipes.
   */
  ...
  def setPipeDescriptions(p: Pipe, descriptions: Seq[String]): Pipe = {
    p.getStepConfigDef()
      .setProperty(Config.PipeDescriptions, encodePipeDescriptions(getPipeDescriptions(p) ++ descriptions))
    p
  }
...
/**
 * This is an enrichment-pattern class for cascading.pipe.Pipe. The rule is to never use this class directly
 * in input or return types, but only to add methods to Pipe.
 */
class RichPipe(val pipe: Pipe) extends java.io.Serializable with JoinAlgorithms {
  ...
  /**
   * Beginning of block with access to expensive nonserializable state. The state object should contain a
   * function release() for resource management purpose.
   */
  def using[C <: { def release(): Unit }](bf: => C) = new {
    ...
    def map[A, T](
        fs: (Fields, Fields)
    )(fn: (C, A) => T)(implicit conv: TupleConverter[A], set: TupleSetter[T]) = {
    ...
    def flatMap[A, T](
        fs: (Fields, Fields)
    )(fn: (C, A) => TraversableOnce[T])(implicit conv: TupleConverter[A], set: TupleSetter[T]) = {
      conv.assertArityMatches(fs._1)
      set.assertArityMatches(fs._2)
  ...
  /**
   * Keep only the given fields, and discard the rest. takes any number of parameters as long as we can
   * convert them to a fields object
   */
   ...
   *
   * builder is typically a block that modifies the given GroupBuilder the final OUTPUT of the block is used
   * to schedule the new pipe each method in GroupBuilder returns this, so it is recommended to chain them and
  ...
  /**
   * Group all tuples down to one reducer. (due to cascading limitation). This is probably only useful just
   * before setting a tail such as Database tail, so that only one reducer talks to the DB. Kind of a hack.
   */
   ...
   *
   * Just about the only reasonable case of this method is to reduce all values of a column or count all the
   * rows.
   */
  ...
  /**
   * Force a random shuffle of all the data to exactly n reducers, with a given seed if you need
   * repeatability.
   */
   ...
   *
   * ==Warning==
   * `rename('x,'y)` is interpreted by scala as `rename(Tuple2('x,'y))` which then does `rename('x -> 'y)`.
   * This is probably not what is intended but the compiler doesn't resolve the ambiguity. YOU MUST CALL THIS
   * WITH A TUPLE2! If you don't, expect the unexpected.
   */
  ...
  /**
   * Text files can have corrupted data. If you use this function and a cascading trap you can filter out
   * corrupted data from your pipe.
   */
  def verifyTypes[A](f: Fields)(implicit conv: TupleConverter[A]): Pipe =
    pipe.filter(f)((a: A) => true)

  /**
   * Given a function, partitions the pipe into several groups based on the output of the function. Then
   * applies a GroupBuilder function on each of the groups.
   *
   * Example: pipe .mapTo(()->('age, 'weight) { ... } .partition('age -> 'isAdult) { _ > 18 } {
   * _.average('weight) } pipe now contains the average weights of adults and minors.
   */
  def partition[A, R](fs: (Fields, Fields))(fn: (A) => R)(
      builder: GroupBuilder => GroupBuilder
  )(implicit conv: TupleConverter[A], ord: Ordering[R], rset: TupleSetter[R]): Pipe = {
  ...
  /**
   * If you use a map function that does not accept TupleEntry args, which is the common case, an implicit
   * conversion in GeneratedConversions will convert your function into a `(TupleEntry => T)`. The result type
   * T is converted to a cascading Tuple by an implicit `TupleSetter[T]`. acceptable T types are primitive
   * types, cascading Tuples of those types, or `scala.Tuple(1-22)` of those types.
   *
   * After the map, the input arguments will be set to the output of the map, so following with filter or map
   * is fine without a new using statement if you mean to operate on the output.
   *
   * {{{
   * map('data -> 'stuff)
   * }}}
   *
   * * if output equals input, REPLACE is used. * if output or input is a subset of the other SWAP is used. *
  ...
  def map[A, T](
      fs: (Fields, Fields)
  )(fn: A => T)(implicit conv: TupleConverter[A], setter: TupleSetter[T]): Pipe = {
    conv.assertArityMatches(fs._1)
  ...
  def flatMap[A, T](
      fs: (Fields, Fields)
  )(fn: A => TraversableOnce[T])(implicit conv: TupleConverter[A], setter: TupleSetter[T]): Pipe = {
    conv.assertArityMatches(fs._1)
    setter.assertArityMatches(fs._2)
  ...
  def collect[A, T](
      fs: (Fields, Fields)
  )(fn: PartialFunction[A, T])(implicit conv: TupleConverter[A], setter: TupleSetter[T]): Pipe = {
    conv.assertArityMatches(fs._1)
    setter.assertArityMatches(fs._2)
  ...
  /**
   * Force a materialization to disk in the flow. This is useful before crossWithTiny if you filter just
   * before. Ideally scalding/cascading would see this (and may in future versions), but for now it is here to
   * aid in hand-tuning jobs
   */
   ...
   * Only the columns given as input fields are expanded in this way. For this operation to be reversible, you
   * need to keep some unique key on each row. See GroupBuilder.pivot to reverse this operation assuming you
   * leave behind a grouping key
   * ==Example==
   * {{{
  ...
  /**
   * Keep at most n elements. This is implemented by keeping approximately n/k elements on each of the k
   * mappers or reducers (whichever we wind up being scheduled on).
   */
  def limit(n: Long): Pipe = new Each(pipe, new Limit(n))

  /**
   * Sample a fraction of elements. fraction should be between 0.00 (0%) and 1.00 (100%) you can provide a
  ...
  /**
   * Sample fraction of elements with return. fraction should be between 0.00 (0%) and 1.00 (100%) you can
   * provide a seed to get reproducible results
   */
  ...
  def sampleWithReplacement(fraction: Double, seed: Int): Pipe =
    new Each(pipe, new SampleWithReplacement(fraction, seed), Fields.ALL)
  ...
  /**
   * Adds a trap to the current pipe, which will capture all exceptions that occur in this pipe and save them
   * to the trapsource given
   *
   * Traps do not include the original fields in a tuple, only the fields seen in an operation. Traps also do
   * not include any exception information.
   *
   ...
   *
   * If those assumptions do not hold, will throw an exception -- consider checking sum sepsarately and/or
   * using addTrap
   *
   ...
   * }}}
   *
   * will pack fields 'field1 and 'field2 to field 'field3, as long as 'field1 and 'field2 can be cast into
   * integers. The output field 'field3 will be of tupel `(Int, Int)`
   */
  ...
  /**
   * This finds all the boxed serializations stored in the flow state map for this flowdef. We then find all
   * the pipes back in the DAG from this pipe and apply those serializations.
   */
  private[scalding] def applyFlowConfigProperties(flowDef: FlowDef): Pipe = {
    case class ToVisit[T](queue: Queue[T], inQueue: Set[T]) {
      def maybeAdd(t: T): ToVisit[T] = if (inQueue(t)) this
      else {
