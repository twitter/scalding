...
/**
 * This controls the sequence of reductions that happen inside a particular grouping operation. Not all
 * elements can be combined, for instance, a scanLeft/foldLeft generally requires a sorting but such sorts are
 * (at least for now) incompatible with doing a combine which includes some map-side reductions.
 */
class GroupBuilder(val groupFields: Fields)
  ...
  /**
   * Holds the number of reducers to use in the reduce stage of the groupBy/aggregateBy. By default uses
   * whatever value is set in the jobConf.
   */
  ...
  /**
   * Prefer reduce or mapReduceMap. foldLeft will force all work to be done on the reducers. If your function
   * is not associative and commutative, foldLeft may be required.
   *
   * ==Best Practice==
   * Make sure init is an immutable object.
   *
   * ==Note==
   * Init needs to be serializable with Kryo (because we copy it for each grouping to avoid possible errors
   * using a mutable init object).
   */
  def foldLeft[X, T](
  ...
  /**
   * Corresponds to a Cascading Buffer which allows you to stream through the data, keeping some, dropping,
   * scanning, etc... The iterator you are passed is lazy, and mapping will not trigger the entire evaluation.
   * If you convert to a list (i.e. to reverse), you need to be aware that memory constraints may become an
   * issue.
   *
   * ==Warning==
   * Any fields not referenced by the input fields will be aligned to the first output, and the final hadoop
   * stream will have a length of the maximum of the output of this, and the input stream. So, if you change
   * the length of your inputs, the other fields won't be aligned. YOU NEED TO INCLUDE ALL THE FIELDS YOU WANT
   * TO KEEP ALIGNED IN THIS MAPPING! POB: This appears to be a Cascading design decision.
   *
   * ==Warning==
  ...
  /**
   * Analog of standard scanLeft (@see scala.collection.Iterable.scanLeft ) This invalidates map-side
   * aggregation, forces all data to be transferred to reducers. Use only if you REALLY have to.
   *
   * ==Best Practice==
   * Make sure init is an immutable object.
   *
   * ==Note==
   * init needs to be serializable with Kryo (because we copy it for each grouping to avoid possible errors
   * using a mutable init object). We override the default implementation here to use Kryo to serialize the
   * initial value, for immutable serializable inits, this is not needed
   */
  override def scanLeft[X, T](
      fieldDef: (Fields, Fields)
  )(init: X)(fn: (X, T) => X)(implicit setter: TupleSetter[X], conv: TupleConverter[T]): GroupBuilder = {
    val (inFields, outFields) = fieldDef
    // Check arity
...
/**
 * Scala 2.8 Iterators don't support scanLeft so we have to reimplement The Scala 2.9 implementation creates
 * an off-by-one bug with the unused fields in the Fields API
 */
