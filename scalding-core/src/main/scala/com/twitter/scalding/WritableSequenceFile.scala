...
trait WritableSequenceFileScheme extends SchemedSource {
  ...
  /**
   * There are three allowed cases: fields.size == 1 and keyType == null fields.size == 1 and valueType ==
   * null fields.size == 2 and keyType != null and valueType != null
   */
  def fields: Fields
...
/**
 * This is only a TypedSource (which is a superclass of Mappable) as sinking into multiple directories is not
 * well defined
 */
