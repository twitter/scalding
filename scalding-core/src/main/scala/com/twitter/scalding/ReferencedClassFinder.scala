...
object ReferencedClassFinder {
   ...
   * https://issues.scala-lang.org/browse/SI-10129), we can't iterate over scalaType.members, so we instead
   * use java reflection to iterate over fields to find the ones we care about, and then look those up in
   * scala reflection to find the full un-erased type signatures, and try to find types from those.
   *
  ...
  private def getFieldType(
      ...
      field: jReflect.Field
  ): Option[universe.Type] =
  ...
  private def safeScalaReflectionCall[T](outerClass: Class[_])(call: => T): Option[T] =
    try {
      ...
      case r: RuntimeException if r.getMessage.contains("error reading Scala signature") =>
        LOG.warn(
          s"Unable to find referenced classes for: $outerClass. This is potentially due to missing dependencies",
          r
        )
        None
      case t: Throwable if t.getMessage.contains("illegal cyclic reference") =>
        ...
        LOG.warn(
          s"Unable to find referenced classes for: $outerClass. Related to Scala language issue: SI-10129",
          t
        )
        None
  ...
  private def getClassesForType(mirror: RuntimeMirror, typeSignature: Type): Seq[Class[_]] =
    typeSignature match {
