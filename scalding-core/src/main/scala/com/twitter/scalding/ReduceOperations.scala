...
import com.twitter.algebird.{Aggregator, AveragedValue, HLL, HyperLogLogMonoid, Moments, Ring, Semigroup}
...
/**
 * Implements reductions on top of a simple abstraction for the Fields-API This is for associative and
 * commutive operations (particularly Monoids and Semigroups play a big role here)
 *
...
trait ReduceOperations[+Self <: ReduceOperations[Self]] extends java.io.Serializable {
  ...
  /**
   * Type T is the type of the input field (input to map, T => X) Type X is the intermediate type, which your
   * reduce function operates on (reduce is (X,X) => X) Type U is the final result type, (final map is: X =>
   * U)
   *
   * The previous output goes into the reduce function on the left, like foldLeft, so if your operation is
   * faster for the accumulator to be on one side, be aware.
   *
   * Assumed to be a commutative operation. If you don't want that, use .forceToReducers
   */
  def mapReduceMap[T, X, U](
      fieldDef: (Fields, Fields)
  )(mapfn: T => X)(redfn: (X, X) => X)(mapfn2: X => U)(implicit
   ...
   * ==Similar To==
   * <a
   * href="http://en.wikipedia.org/wiki/Algorithms_for_calculating_variance#Parallel_algorithm">http://en.wikipedia.org/wiki/Algorithms_for_calculating_variance#Parallel_algorithm</a>
   */
  def average(f: (Fields, Fields)) = mapPlusMap(f)((x: Double) => AveragedValue(1L, x))(_.value)
  def average(f: Symbol): Self = average(f -> f)

  /**
   * Approximate number of unique values We use about m = (104/errPercent)^2 bytes of memory per key Uses
   * `.toString.getBytes` to serialize the data so you MUST ensure that .toString is an equivalance on your
   * counted fields (i.e. `x.toString == y.toString` if and only if `x == y`)
   *
   * For each key:
   * {{{
   * 10% error ~ 256 bytes
  ...
  /**
   * Opposite of RichPipe.unpivot. See SQL/Excel for more on this function converts a row-wise representation
   * into a column-wise one.
   *
   * ==Example==
   * {{{
   ...
   * }}}
   *
   * and you will have non-nulls for all present values, and Nones for values that were present but previously
   * null. All nulls in the final output will be those truly missing. Similarly, if you want to check if there
   * are any items present that shouldn't be:
   *
   * {{{
  ...
  /**
   * Compute the count, ave and standard deviation in one pass example: g.sizeAveStdev('x -> ('cntx, 'avex,
   * 'stdevx))
   */
  ...
  /**
   * Collect all the values into a List[T] and then operate on that list. This fundamentally uses as much
   * memory as it takes to store the list. This gives you the list in the reverse order it was encounted (it
   * is built as a stack for efficiency reasons). If you care about order, call .reverse in your fn
   *
  ...
  /**
   * Similar to the scala.collection.Iterable.mkString takes the source and destination fieldname, which
   * should be a single field. The result will be start, each item.toString separated by sep, followed by end
  ...
  /**
   * Use `Semigroup.plus` to compute a sum. Not called sum to avoid conflicting with standard sum Your
   * `Semigroup[T]` should be associated and commutative, else this doesn't make sense
   *
   * Assumed to be a commutative operation. If you don't want that, use .forceToReducers
   */
  def sum[T](
  ...
  /**
   * Equivalent to sorting by a comparison function then take-ing k items. This is MUCH more efficient than
   * doing a total sort followed by a take, since these bounded sorts are done on the mapper, so only a sort
   * of size k is needed.
   *
   * ==Example==
   * {{{
  ...
  def histogram(f: (Fields, Fields), binWidth: Double = 1.0) =
    mapPlusMap(f) { x: Double => Map((math.floor(x / binWidth) * binWidth) -> 1L) } { map =>
