...
case class GlobifierOps(implicit tz: TimeZone, dp: DateParser) {
  ...
  def rtGlobifier(globifier: Globifier, durationList: List[Duration])(
      inputDr: DateRange): DateRange = {
    val p = globifier.globify(inputDr)

    val drList = p
      .map { pattern =>
        val (lists, _, _) =
          pattern.split("/").tail.foldLeft((List[(Duration, Duration)](), durationList, true)) {
                ...
                val (duration, doContinue) = tryDuration match {
                  ...
                  case Failure(e) =>
                    val dur: Duration = curMapping match {
                      case Years(_) =>
                        sys.error("Current is " + current + ", parsed as all years?")
