    ...
    "truncate to a minute" in {
      assert(
        dateTimeParser.parse("February 12, 2002 12:34:00.000") ===
          CalendarOps.truncate(dateTimeParser.parse("February 12, 2002 12:34:56.789"),
                               Calendar.MINUTE))

      assert(
        dateTimeParser.parse("November 18, 2001 1:23:00.000") ===
          CalendarOps.truncate(dateTimeParser.parse("November 18, 2001 1:23:11.321"),
                               Calendar.MINUTE))
    }
    ...
    "truncate to AM" in {
      assert(
        dateTimeParser.parse("February 3, 2002 00:00:00.000") ===
          CalendarOps.truncate(dateTimeParser.parse("February 3, 2002 01:10:00.000"),
                               Calendar.AM_PM))

      assert(
        dateTimeParser.parse("February 3, 2002 00:00:00.000") ===
          CalendarOps.truncate(dateTimeParser.parse("February 3, 2002 11:10:00.000"),
                               Calendar.AM_PM))
    }
    ...
    "truncate to PM" in {
      assert(
        dateTimeParser.parse("February 3, 2002 12:00:00.000") ===
          CalendarOps.truncate(dateTimeParser.parse("February 3, 2002 13:10:00.000"),
                               Calendar.AM_PM))

      assert(
        dateTimeParser.parse("February 3, 2002 12:00:00.000") ===
          CalendarOps.truncate(dateTimeParser.parse("February 3, 2002 19:10:00.000"),
                               Calendar.AM_PM))
    }
