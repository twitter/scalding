...
/**
 * Represents millisecond based duration (non-calendar based): seconds, minutes, hours calField should be a
...
abstract class Duration(val calField: Int, val count: Int, val tz: TimeZone) extends java.io.Serializable {
...
case class Days(cnt: Int)(implicit tz: TimeZone) extends Duration(Calendar.DAY_OF_MONTH, cnt, tz)

case class Weeks(cnt: Int)(implicit tz: TimeZone) extends Duration(Calendar.WEEK_OF_YEAR, cnt, tz) {
...
case class Months(cnt: Int)(implicit tz: TimeZone) extends Duration(Calendar.MONTH, cnt, tz)

case class Years(cnt: Int)(implicit tz: TimeZone) extends Duration(Calendar.YEAR, cnt, tz)
