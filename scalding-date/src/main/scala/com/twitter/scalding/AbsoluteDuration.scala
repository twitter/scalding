...
object AbsoluteDuration extends java.io.Serializable {
  ...
  val UTC_UNITS =
    List[TimeCons]((Hours, HOUR_IN_MS), (Minutes, MIN_IN_MS), (Seconds, SEC_IN_MS), (Millisecs, 1)).reverse
...
final case class Millisecs(cnt: Int)
...
final case class Seconds(cnt: Int) extends Duration(Calendar.SECOND, cnt, DateOps.UTC) with AbsoluteDuration {
  override def toSeconds = cnt.toDouble
  override def toMillisecs = (cnt.toLong) * 1000L
}

final case class Minutes(cnt: Int) extends Duration(Calendar.MINUTE, cnt, DateOps.UTC) with AbsoluteDuration {
  override def toSeconds = cnt * 60.0
  override def toMillisecs = cnt.toLong * 60L * 1000L
}

final case class Hours(cnt: Int) extends Duration(Calendar.HOUR, cnt, DateOps.UTC) with AbsoluteDuration {
