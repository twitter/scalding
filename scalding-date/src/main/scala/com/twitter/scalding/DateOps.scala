...
object DateOps extends java.io.Serializable {
  ...
  private[scalding] object Format {
    ...
    case object DATEHOUR_WITHOUT_DASH
        extends Format(DateOps.DATEHOUR_WITHOUT_DASH, new Regex(emptyBegin + """\d{10}""" + emptyEnd))
    case object DATEHOUR_WITH_DASH
        extends Format(DateOps.DATEHOUR_WITH_DASH, new Regex(emptyBegin + date + sep + """\d\d""" + emptyEnd))
    case object DATETIME_WITHOUT_DASH
        extends Format(DateOps.DATETIME_WITHOUT_DASH, new Regex(emptyBegin + """\d{12}""" + emptyEnd))
    ...
    case object DATETIME_HMS_WITHOUT_DASH
        extends Format(DateOps.DATETIME_HMS_WITHOUT_DASH, new Regex(emptyBegin + """\d{14}""" + emptyEnd))
    ...
    case object DATETIME_HMSM_WITH_DASH
        extends Format(
          ...
          new Regex(emptyBegin + date + sep + """\d\d:\d\d:\d\d\.\d{1,3}""" + emptyEnd)
        )
  }
  ...
  /**
   * The DateParser returned here is based on SimpleDateFormat, which is not thread-safe. Do not share the
