...
object DateRange extends java.io.Serializable {
  ...
  /**
   * Parse this string into a range. 2009-10-01 is interpetted as the whole day 2009-10-01T12 is interpetted
   * as the whole hour 2009-10-01T12:00 is interpetted as a single minute 2009-10-01T12:00:02 is interpretted
   * as a single second
   *
  ...
  /**
   * We take the upper bound of the second parameter, so we take the latest time that could be construed as
   * matching the string passed, e.g. ("2011-01-02T04", "2011-01-02T05") includes two full hours (all of 4 and
   * all of 5)
   */
  ...
  /**
   * DateRanges are inclusive. Use this to create a DateRange that excludes the last millisecond from the
   * second argument.
   */
...
case class DateRange(val start: RichDate, val end: RichDate) {
  ...
  /**
   * Extend the length by moving the start. Turns out, we can start the party early.
   */
  ...
  /**
   * produce a contiguous non-overlapping set of DateRanges whose union is equivalent to this. If it is passed
   * an integral unit of time (not a DurationList), it stops at boundaries which are set by the start
