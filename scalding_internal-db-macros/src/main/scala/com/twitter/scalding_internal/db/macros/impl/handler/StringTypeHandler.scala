  ...
  def apply[T](c: Context)(implicit fieldName: FieldName,
    ...
    extracted.flatMap { t =>
      t match {
        ...
        case (WithSize(siz), WithoutVarchar, WithoutText) if siz <= 255 => Success(List(ColFormatter(c)("VARCHAR", Some(siz))))
        case (WithSize(siz), WithoutVarchar, WithoutText) if siz > 255 => Success(List(ColFormatter(c)("TEXT", None)))
        case (WithSize(siz), WithVarchar, WithoutText) => Success(List(ColFormatter(c)("VARCHAR", Some(siz))))
        case (_, WithoutVarchar, WithText) => Success(List(ColFormatter(c)("TEXT", None)))
      }
    }
  }
}
