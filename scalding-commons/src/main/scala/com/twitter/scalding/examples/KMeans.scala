...
object KMeans {
  ...
  private def closest[Id](
      from: Vector[Double],
      centroids: TraversableOnce[(Id, Vector[Double])]
  ): (Id, Vector[Double]) =
    centroids
  ...
  /**
   * This runs one step in a kmeans algorithm It returns the number of vectors that changed clusters, the new
   * clusters and the new list of labeled vectors
   */
  def kmeansStep(
      k: Int,
      s: Stat,
  ...
  def initializeClusters(
      k: Int,
      points: TypedPipe[Vector[Double]]
  ...
  def kmeans(
      ...
      clusters: ValuePipe[List[LabeledVector]],
      points: TypedPipe[LabeledVector]
  ): Execution[(Int, ValuePipe[List[LabeledVector]], TypedPipe[LabeledVector])] = {
