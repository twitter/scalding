...
class TypedWriteIncrementalJob(args: Args) extends Job(args) {
  ...
  implicit val inj: Injection[(Int, Int), (Array[Byte], Array[Byte])] =
    Injection.connect[(Int, Int), (Array[Byte], Array[Byte])]
...
class VersionedKeyValSourceTest extends WordSpec with Matchers {
      ...
      .sink[(Int, Int)](VersionedKeyValSource[Array[Byte], Array[Byte]]("output")) {
        outputBuffer: Buffer[(Int, Int)] =>
          "Outputs must be as expected" in {
            ...
            assert(
              input.map(k => (k, k)).sortBy(_._1).toString === outputBuffer.sortBy(_._1).toList.toString
            )
          }
      }
      .run
  ...
  "A VersionedKeyValSource" should {
    ...
    "calculate right size of source" in {
      ...
      val path = setupLocalVersionStore(
        100L to 102L,
        {
   ...
   */
  private def setupLocalVersionStore(
      versions: Seq[Long],
      contentFn: Long => Option[String] = _ => None
  ): String = {
  ...
  /**
   * Creates a VersionedKeyValSource using the provided version and then validates it.
   */
